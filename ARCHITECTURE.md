...
Use [docs/constraints-index.md](docs/constraints-index.md) as the quick entry point for the complete constraint file set. It links the active plan, architecture guardrails, engineering workflow, clean repository rules, and demo runbooks.

For future feature placement and anti-drift rules, use [docs/architecture-guardrails.md](docs/architecture-guardrails.md) with this file. For the approved backlog direction, use [docs/plan.md](docs/plan.md).
...
lexproof-ai-audit/
  ...
  src/
    ...
    lib/
      ...
      documentParserPolicyClient.ts # Browser client for document parser policy evaluation
      chainAnchorPolicy.ts # Metadata-only simulated anchor policy readiness reports and JSON export
      chainAnchorPolicyClient.ts # Browser client for chain anchor policy evaluation
...
demoScenarios, sampleProfiles, or blank project
  ...
  -> createDocumentParserPolicyReport(retention/export context, manifest hash, parser policy draft)
  -> createChainAnchorPolicyReport(retention/export context, manifest hash, Counsel Pack version count, anchor policy draft)
  -> createGrcDestinationPolicyReport(remediation/export/adapter context, GRC destination policy draft)
...
### `src/lib/jurisdictionEvidenceMap.ts`

Owns per-jurisdiction evidence-map workflow metadata for the Regulatory Command Center:

- `createJurisdictionEvidenceMap({ matrix })` groups control-matrix rows by jurisdiction, summarizes source topics, local counsel roles, open evidence requests, highest priority, next actions, and a stable SHA-256 map hash.
- Map status is workflow-only: `needs-evidence`, `needs-source-review`, `metadata-missing`, `ready-for-counsel`, or `no-controls`.
...
### `src/lib/grcDestinationPolicy.ts`

Owns the W9 destination-adapter policy gate before any future ticket-system sync:

- `createGrcDestinationPolicyReport(input)` turns remediation queue state, Export Safety state, local GRC ticket export readiness, destination scope, field mapping, authentication policy, redaction policy, ownership, retry/audit logging, and human-review enforcement into a metadata-only readiness report.
- `exportGrcDestinationPolicyJson()` serializes the report for review handoff.
- `fetchGrcDestinationPolicyReport()` in `grcDestinationPolicyClient.ts` posts only whitelisted context and policy fields from the workbench and rejects malformed responses that try to enable external ticket creation.

The report is audit preparation workflow metadata only. It does not collect API keys or webhook secrets, send raw ticket bodies, create Jira/Linear/ServiceNow/GRC records, or turn review metadata into legal advice.
...
- derive document-parser readiness from Evidence Retention status, Export Safety blockers, Evidence Manifest hash, bounded document size, raw-document retention, deletion SLA, parser purpose, redaction-before-parsing, no-training-use, access logging, no-sensitive-material confirmation, and human-review enforcement
- derive GRC destination readiness from remediation queue state, Export Safety blockers, local ticket export readiness, destination scope, field mapping, authentication policy, redaction policy, ownership, retry/audit logging, no-sensitive-material confirmation, and human-review enforcement
...
`src/lib/documentParserPolicy.ts` owns the document-parser policy report and JSON export. `src/lib/documentParserPolicyClient.ts` posts only whitelisted context and policy fields from the workbench. `server/integrationPolicyRoutes.ts` exposes `POST /api/integrations/document-parser/policy`, sanitizes request fields, and returns a Not legal advice report without storing records, accepting raw document bytes or raw document bodies, executing OCR, collecting credentials, enabling parser adapters, or making legal decisions.

`src/lib/grcDestinationPolicy.ts` owns the GRC destination policy report and JSON export. `src/lib/grcDestinationPolicyClient.ts` posts only whitelisted context and policy fields from the workbench: workspace/remediation/export/adapter metadata plus policy owner, destination system, destination queue, field mapping approval, authentication policy approval, redaction policy approval, ticket ownership approval, retry/audit logging approval, no-sensitive-material confirmation, human-review enforcement, and notes. `server/integrationPolicyRoutes.ts` exposes `POST /api/integrations/grc-destination/policy`, sanitizes request fields, blocks unsafe metadata such as credentials, private keys, raw KYC, personal data, raw ticket body references, and external ticket-write requests, and returns a Not legal advice report without API keys, webhook secrets, raw ticket payloads, external ticket creation, or real GRC system writes.
...
- regulatory source review freshness, source update approval gates, jurisdiction evidence maps, review-due actions, reviewer notes, and non-advice boundary
