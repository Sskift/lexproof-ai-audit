...
- Server Model Gateway provider policy API, secret policy evaluation API, workbench refresh flow, receipts, automatic Human Review queueing for completed gateway output, Model Gateway Evaluation artifacts, workspace-level run recovery packets, and Audit Log Export artifacts with payload hash, response hash, source evidence hash, provider/secret policy metadata, human-review status, retry/error state, audit actions, before/after hashes, and remediation steps without returning raw payloads or credentials.
...
- Integration Enablement Dossier and Integration Enablement Gate inside Integration Readiness, with downloadable metadata-only JSON artifacts that consolidate adapter readiness, provider/secret/storage/parser/chain/GRC policy reports, server policy receipt coverage, blockers, next actions, stable dossier/gate hashes, and `externalEnablementAllowed: false`.
...
- Integration Policy Evaluation Receipts for server-evaluated object storage, document parser, chain anchor, and GRC destination policy reports, persisted by workspace with report/context/policy hashes, audit-log entries, an explicit metadata-only Not legal advice boundary, a downloadable receipt bundle hash, and a server receipt recovery packet for missing, blocked, needs-policy, or stale receipt states before adapter enablement review.
...
- Regulatory Command Center first screen with a Workspace Cockpit Handoff JSON, Workspace Journey rail, Workspace Action Queue, final handoff recovery routing to Sources, Regulatory Control Matrix, Jurisdiction Evidence Map, Jurisdiction Readiness Digest, Source Freshness Board, Local Counsel Routing Plan, jurisdiction readiness, source-backed clause triggers, Source Review Ledger metadata, Source Update Approval Queue, evidence gaps, manifest readiness, and counsel handoff status.
- Regulatory Source Graph for official-source audit-prep triggers across US SEC/CFTC, US SEC Regulation D accredited-investor review, US SEC Investment Adviser Marketing Rule review, US OFAC virtual-currency sanctions guidance, US FinCEN CVC/MSB and BSA transfer-recordkeeping review, US GENIUS Act payment stablecoin issuer review, US SEC DAO Report, US CFTC Ooki DAO / DeFi derivatives-platform review, US FTC advertising/endorsement guides, ABA Formal Opinion 512 legal AI professional-responsibility review, NYC Local Law 144 AEDT hiring/promotion review, California CCPA ADMT consumer-rights review, Colorado SB26-189 ADMT consequential-decision review, EU MiCA Title II, ART/EMT stablecoin issuer review, decentralised DAO / CASP perimeter review, Article 75 CASP custody, and marketing communications, EU DORA ICT operational resilience, EU TFR crypto-asset transfer information review, EU DLT Pilot Regime market-infrastructure review, EU AI Act AI-literacy, high-risk provider quality-system / technical-documentation review, Article 50 transparency, and administration-of-justice/ADR perimeter review, UK FCA qualifying stablecoin issuer review, UK FCA financial promotions, UK FCA cryptoasset AML/MLR registration and Travel Rule guidance, UK Law Commission DAO scoping, UK ICO AI data protection guidance, Singapore IMDA / AI Verify Foundation Agentic AI governance review, Singapore MAS PSN02 and PS-G03 DPT customer-asset safeguards, Hong Kong SFC VATP custody, Hong Kong HKMA stablecoin issuer regime, Hong Kong SFC tokenised authorised investment product and secondary-trading review, Japan FSA crypto-asset custody/user-protection guidance, Canada CSA CTP custody/investor-protection guidance, Australia ASIC digital-asset financial-services and AUSTRAC VASP AML/CTF guidance, South Korea FSC Virtual Asset User Protection Act / Enforcement Decree and KoFIU VASP reporting / AML guidance, India FIU-IND / Ministry of Finance PMLA VDA AML/CFT guidance, Thailand SEC digital asset business custody / AMLO AML/CDD review, Indonesia OJK digital financial asset / crypto asset trading review, Malaysia SC digital asset exchange/custody and BNM AML/CFT review, Philippines BSP VASP/CASP risk-management review, South Africa FSCA/FIC CASP licensing and Travel Rule review, German BaFin/MiCAR CASP custody review, Swiss FINMA ICO and stablecoin guidance, UAE VARA activity scope / compliance / 2024 marketing regulations, Brazil Banco Central, and Brazil CVM references, including effective dates, source review dates, and reviewer notes.
...
- Downloadable metadata-only Source Review Packet JSON plus server Source Review Packet refresh/download from the command center with source refresh actions, clause-match targets, ledger hashes, packet hash, and Not legal advice boundary.
...
- Server-syncable metadata-only Source Review Ledger from the command center through `POST /api/workspaces/:workspaceId/source-reviews` and server packet refresh through `GET /api/workspaces/:workspaceId/source-reviews/packet`, with stable ledger/packet hashes, audit-log metadata, and `matchingBehaviorChanged: false`.
- Downloadable and server-syncable metadata-only Source Update Approval Queue from the command center with review-due source approvals, metadata gates, `POST /api/workspaces/:workspaceId/source-approvals`, server packet hashes through `GET /api/workspaces/:workspaceId/source-approvals/packet`, audit-log metadata, and a boundary that source updates cannot affect matching until counsel or compliance review records refreshed source metadata.
...
- Evidence Vault versioning with duplicate-hash detection, rejected-record replacement, parent/superseded lineage, replacement reasons, linked regulatory control IDs, server-side persisted metadata manifests, downloadable Evidence Vault Lineage Digest JSON, a server Evidence Vault Lineage Recovery Packet, server-enforced status transitions, local `under-review/rejected` ledger sync, and metadata-only recovery actions.
- Evidence Templates for tokenized yield/RWA issuance, DAO governance/multisig execution, and AI legal/compliance workflows, including EU AI Act high-risk provider QMS and technical-documentation evidence links.
- Evidence Manifest generator with deterministic SHA-256 item hashes, bundle hash, JSON download, and a Manifest Drift Guard that compares the current manifest with refreshed Evidence Vault hash lineage, the latest Counsel Pack version, and the latest server export record before handoff.
...
- Counsel Pack version history with manifest hash, Markdown hash, source snapshot, Regulatory Source Pack hash, source review status, Jurisdiction Readiness Digest hash/status/handoff flag, review status summary, diff from the previous export, metadata-only version JSON download, and standalone diff JSON download.
- Server-side Counsel Pack export records through the Phase 2 API, storing export hashes, version metadata, review summary, source count, Regulatory Source Pack hash, source review status, Jurisdiction Readiness Digest hash/status/handoff flag, and audit-log entries without raw Markdown, KYC, personal data, or credentials. Each displayed server export row can download a metadata-only Export Record Receipt JSON with a stable receipt hash, recovery action, jurisdiction readiness metadata, and Not legal advice boundary. The workbench can refresh the metadata-only Counsel Pack export recovery packet directly from `GET /api/workspaces/:workspaceId/exports/counsel-pack/recovery`, then download stable packet hashes, stale-source/review-blocker counts, and validated non-empty recovery actions across saved export records. Blocked data-boundary findings disable Markdown/PDF, manifest JSON, simulated anchor, version save, and server export actions until remediated.
- Counsel Handoff Checklist in Counsel Pack, with a stable checklist hash, export safety status, evidence manifest/source pack/submission pack readiness, Manifest Drift Guard status, evidence recertification status, Evidence Vault lineage readiness, review status, saved version state, server export-record state, and downloadable metadata-only JSON. Not legal advice.
...
- Export Safety Inventory in Sources with a stable inventory hash, artifact readiness, Manifest Drift Guard hash/status, Model Gateway Evaluation hash/status when available, Demo Runbook JSON, Source Freshness Board hash/status, Source Update Approval Queue hash/status, Evidence Vault Lineage Digest hash/status, Evidence Disposal Runbook hash/status, and Integration Enablement Gate hash/status when available, data-boundary blockers, metadata-only JSON download, and an explicit handoff allowed/blocked status before counsel or judge artifacts leave the workspace.
...
The DAO proposal scenario routes `ClauseGuard DAO` into US SEC DAO Report, US CFTC Ooki DAO / DeFi derivatives-platform, EU MiCA decentralised DAO / CASP perimeter, and UK Law Commission DAO scoping source controls for governance-token, participant-role, voting, signer, derivatives-platform boundary, FCM/BSA/CIP assumptions, EU user-access and intermediary-control evidence, and asset-control evidence review. Not legal advice.

The Jurisdiction Checklist also shows a US SEC DAO Report governance-token and participant-role control that becomes evidence-ready only from metadata-only DAO proposal, signer-authority, vote/execution, and contributor-summary records carrying `control-us-sec-dao-report-governance-token-review`, without raw customer records or legal conclusions. Not legal advice.

The Jurisdiction Checklist also shows a US CFTC Ooki DAO derivatives-platform and FCM/BSA control that becomes evidence-ready only from the metadata-only DAO derivatives-platform boundary register carrying `control-us-cftc-ooki-dao-defi-derivatives-platform`, including leveraged or margined retail commodity assumptions, DeFi trading-platform boundary, FCM activity, commodity-interest handling, BSA/CIP boundary, control transfer, governance-member, website-domain, proposal-execution, and compliance-owner evidence without raw customer records. Not legal advice.

The Jurisdiction Checklist also shows an EU MiCA decentralised DAO and CASP perimeter control that becomes evidence-ready only from the metadata-only DAO perimeter register carrying `control-eu-mica-decentralised-casp-perimeter`, including decentralisation claims, fully decentralised service assumptions, intermediary, operator-control, front-end-control, admin-key, protocol-upgrade, governance-vote, EU user-access, crypto-asset-service perimeter, CASP-perimeter, custody/trading/exchange/advice-service exclusion, marketing-owner, and responsible-owner evidence without raw customer records or personal data. Not legal advice.

The Jurisdiction Checklist also shows a UK Law Commission DAO structure and asset-control scoping control that becomes evidence-ready only from metadata-only DAO proposal, signer-authority, vote/execution, and contributor-summary records carrying `control-uk-law-commission-dao-scoping-paper`, including DAO structure, participant-role, contributor, foundation, legal-characterisation, governance-rule, quorum, voting-window, multisig-signer, asset-control, execution-receipt, and emergency-authority evidence without raw customer records or legal conclusions. Not legal advice.

![DAO governance source graph](docs/assets/screenshots/dao-governance-source-graph.png)

The AI legal workflow scenario now routes `LexAssist Evidence Desk` into ABA Formal Opinion 512, US NIST AI RMF / NIST AI 600-1 Generative AI Profile, NYC Local Law 144 AEDT hiring/promotion review, California CCPA ADMT consumer-rights review, Colorado SB26-189 ADMT consequential-decision review, EU AI Act AI-literacy, Article 50 transparency, administration-of-justice/ADR perimeter review, UK ICO AI data protection source controls, and Singapore IMDA / AI Verify Foundation Agentic AI governance controls, with evidence gaps for legal-AI competence, confidentiality, client-communication, supervisory review, candor, fee treatment, govern/map/measure/manage evidence, GenAI output provenance, NYC AEDT scope, independent bias audit, public summary, 10-business-day notice, accommodation or alternative-process routing, data/source/retention request handling, California CCPA ADMT scope, risk assessment, access, opt-out, secure response, Colorado ADMT scope, developer documentation, notice, adverse-outcome explanation, meaningful human review, record retention, AI use policy, source lineage, Article 50 user-interaction disclosure, AI-generated output labelling, editorial control, justice/ADR perimeter, high-risk oversight, fundamental-rights review, redaction, reviewer decision logs, agent action-space, tool permissions, human approval checkpoints, logging, monitoring, user responsibility, and AI Verify documentary evidence. The Jurisdiction Checklist also shows US ABA Formal Opinion 512 legal-AI, US NIST AI RMF / GenAI provenance, NYC Local Law 144 AEDT / bias-audit and notice, California CCPA ADMT / risk-assessment and consumer-rights controls, Colorado SB26-189 ADMT / notice and human-review controls, EU AI Act AI-literacy / human-oversight governance, EU AI Act high-risk provider QMS / technical-documentation, EU AI Act Article 50 transparency, EU AI Act justice/ADR perimeter, UK ICO AI data-protection / reviewer-decision, and Singapore IMDA / AI Verify agentic-AI governance controls that become evidence-ready from metadata-only AI use policy, human-review, confidentiality, client-communication, supervision, candor, fee-treatment, NYC-AEDT-scope, independent-bias-audit, public-summary, 10-business-day-notice, alternative-process, data-retention, California-ADMT-scope, significant-decision, risk-assessment-owner, access-request, opt-out-workflow, secure-response, non-retaliation, Colorado-ADMT-scope, technical-documentation, consumer-notice, adverse-outcome-explanation, correction-workflow, meaningful-human-review, three-year-records, Article 4, AI-literacy, human-oversight, source-lineage, use-case context, risk-owner, output-review, source-provenance, content-risk-escalation, provider-role, QMS, risk-management, technical-documentation, data-governance, logging, Article 50 notice, first-interaction timing, output-labelling, editorial-review, Article 6(2), Annex III, ADR, deployer-obligation, fundamental-rights, model-payload-redaction, personal-data-exclusion, reviewer-decision, agent-action-space, tool-permissions, human-approval, tool-call-monitoring, user-responsibility, AI-Verify, no-raw-applicant-records, no-raw-personal-data, and no-confidential-matter-text template records. Not legal advice.

The EU data minimization and model-call control becomes evidence-ready only from metadata-only EU AI Act records carrying `control-eu-ai-act-ai-literacy-governance` or `control-eu-ai-act-high-risk-provider-quality-documentation` plus matching data-governance, retention, redaction, model-call, model-payload, prohibited-input, or personal-data evidence text, without raw personal data, client identifiers, credentials, or legal conclusions. Not legal advice.

![AI workflow regulatory source controls](docs/assets/screenshots/ai-workflow-regulatory-source-controls.png)

![Singapore Agentic AI workflow source control](docs/assets/screenshots/singapore-agentic-ai-workflow-source-control.png)

![US NIST AI workflow source graph](docs/assets/screenshots/us-nist-ai-workflow-source-graph.png)

![NYC AEDT AI workflow source graph](docs/assets/screenshots/nyc-aedt-ai-workflow-source-graph-focused.png)

![California CCPA ADMT AI workflow source graph](docs/assets/screenshots/california-ccpa-admt-ai-workflow-source-graph-focused.png)

![Colorado ADMT AI workflow source graph](docs/assets/screenshots/colorado-admt-ai-workflow-source-graph-focused.png)

The RWA path now routes tokenized private-credit facts into US Regulation D accredited-investor and offering-evidence review triggers, US OFAC virtual-currency sanctions screening and blocked-property escalation review triggers, US FinCEN CVC/MSB and BSA transfer-recordkeeping review triggers, New York NYDFS BitLicense and custody customer-protection review triggers, EU DLT Pilot Regime market-infrastructure perimeter review, and EU projects with platform wallet custody into MiCA Article 75 custody, DORA ICT operational-resilience, and TFR crypto-asset transfer-information review triggers. The Jurisdiction Checklist also shows US Regulation D eligibility, OFAC virtual-currency sanctions, FinCEN CVC/MSB BSA transfer-recordkeeping, NYDFS BitLicense/custody customer-protection, EU MiCA Article 75 CASP custody, EU DORA ICT operational-resilience, EU TFR crypto-asset transfer-information, and EU DLT Pilot market-infrastructure perimeter controls that become evidence-ready from the metadata-only RWA investor eligibility, wallet-screening, CVC transfer-control, NYDFS custody, custody-runbook, DORA ICT resilience, TFR Travel Rule, and DLT Pilot templates. These add investor eligibility, solicitation-control, wallet sanctions screening, blocked-property reporting, CVC business-model and AML-program handoff, New York resident-access and customer virtual-currency segregation handoff, US and EU Travel Rule transfer-recordkeeping handoff, DLT financial-instrument and DLT MTF/TSS/SS perimeter evidence, competent-authority/ESMA handoff metadata, settlement, safekeeping, liability-safeguard evidence, CASP custody policy, client crypto-asset safeguarding, ICT risk-management, incident-response, testing, and ICT third-party service register evidence gaps without making legal conclusions. Not legal advice.

The US NYDFS BitLicense and custody customer-protection control now becomes evidence-ready only from metadata-only records carrying `control-us-nydfs-bitlicense-custody-customer-protection` plus matching New York, NYDFS, BitLicense, limited-purpose-trust-company, NMLS, money-transmission, customer-virtual-currency-segregation, wallet, ledger, reconciliation, beneficial-interest, sub-custody, books-and-records, no-proprietary-use, or reviewer-owner evidence text, so source-free custody runbooks remain needs-evidence. The US custody and wallet authority control also requires that source marker plus matching custody, wallet, segregation, ledger, reconciliation, or sub-custody evidence text, without raw KYC, wallet secrets, customer records, personal data, or legal conclusions. Not legal advice.

The EU MiCA Article 75 CASP custody and administration control becomes evidence-ready only from metadata-only records carrying `control-eu-mica-casp-custody-administration` plus matching MiCA Article 75, CASP, crypto-asset-service-provider, custody-administration, custody-policy, wallet-authority, signer-quorum, means-of-access, return-procedure, client-crypto-asset, safeguarding, withdrawal-approval, emergency-pause, incident-response, reconciliation, delegation, or custody-and-signer-control evidence text, so source-free custody memos remain needs-evidence. Not legal advice.

The EU DORA ICT operational-resilience control becomes evidence-ready only from metadata-only records carrying `control-eu-dora-ict-operational-resilience` plus matching DORA, digital-operational-resilience, ICT-risk, ICT-risk-management, business-continuity, incident-classification, incident-response, escalation-owner, testing-cadence, recovery, ICT-third-party-service-register, critical-function, subcontracting, access-logging, exit-plan, resilience-testing, or vendor-register evidence text, so source-free resilience memos remain needs-evidence. Not legal advice.

The EU TFR crypto-asset transfer-information control becomes evidence-ready only from metadata-only records carrying `control-eu-tfr-crypto-asset-transfer-information` plus matching EU TFR, Regulation EU 2023/1113, crypto-asset-transfer-information, transfer-of-crypto-assets, Travel Rule, travel-rule-transfer-information, counterparty-CASP, originator, beneficiary, missing-incomplete-information, transfer-information-handling, travel-rule-exception, counterparty-escalation, rejection-or-return-handling, retention-owner, or reviewer-owner evidence text, so source-free transfer memos remain needs-evidence. Not legal advice.

The EU DLT Pilot Regime market-infrastructure perimeter control becomes evidence-ready only from metadata-only records carrying `control-eu-dlt-pilot-regime-market-infrastructure` plus matching EU DLT Pilot, Regulation EU 2022/858, DLT-financial-instrument, tokenized-financial-instrument, tokenised-financial-instrument, DLT-market-infrastructure, DLT-MTF, DLT-TSS, DLT-SS, competent-authority, permission-exemption, admitted-instrument, settlement-workflow, safekeeping, liability, operational-safeguard, client-disclosure, ESMA, or no-raw-investor-records evidence text, so source-free market-infrastructure memos remain needs-evidence. Not legal advice.
...
Swiss stablecoin projects now route to FINMA ICO token-classification guidance and FINMA Guidance 06/2024 on stablecoins, with evidence gaps for payment/utility/asset/hybrid token assumptions, offering/prospectus intake, holder redemption claims, reserve assets, bank guarantees, banking-law and collective-investment perimeter assumptions, AML, sanctions, holder identification, transfer risk, and local-counsel handoff. The Jurisdiction Checklist also shows FINMA token classification/prospectus, stablecoin issuer/bank-guarantee, stablecoin AML/sanctions/transfer-risk, and foundation/custody/banking perimeter controls that become evidence-ready only from metadata-only records carrying `control-ch-finma-ico-token-classification` or `control-ch-finma-stablecoin-guidance-06-2024` plus matching token-classification, prospectus, offering-memo, eligibility, disclosure, token-terms, stablecoin-issuer, redemption-claim, stabilisation-mechanism, underlying-assets, bank-guarantee, default-guarantee, banking-law, collective-investment-scheme, stablecoin-AML, money-laundering, terrorist-financing, sanctions-circumvention, holder-identification, transfer-risk, anonymous-transfer, transaction-monitoring, foundation, governance, custody, wallet-control, banking, manifest, or anchor evidence text, so source-free Swiss FINMA stablecoin memos remain needs-evidence. No raw KYC, customer records, credentials, wallet secrets, private cryptographic material, personal data, or legal conclusions. Not legal advice.

![Swiss FINMA stablecoin source graph](docs/assets/screenshots/swiss-finma-stablecoin-source-graph.png)

US payment stablecoin issuer projects now route to GENIUS Act source controls and Treasury/OCC implementation materials, with evidence gaps for permitted-issuer route, state or federal oversight, reserve assets, redemption, monthly disclosure, custody, insolvency-priority evidence, BSA/AML, sanctions, transaction monitoring, and no-raw-KYC handoff. The Jurisdiction Checklist also shows a GENIUS Act payment stablecoin issuer control that becomes evidence-ready only from metadata-only records carrying `control-us-genius-payment-stablecoin-issuer-regime` plus matching GENIUS Act, payment-stablecoin, permitted-issuer, state-qualified, federal-regulator, reserve, redemption, monthly-disclosure, custody, insolvency-priority, BSA/AML, sanctions, transaction-monitoring, suspicious-activity, compliance-officer, customer-risk, or independent-review evidence text, so source-free stablecoin memos remain needs-evidence. Not legal advice.

![US GENIUS Act stablecoin source graph](docs/assets/screenshots/us-genius-stablecoin-source-graph.png)

EU MiCA stablecoin issuer projects now route to ART/EMT source controls from EBA and ESMA MiCA materials, with evidence gaps for token classification, issuer authorisation or notification route, home Member State, competent-authority handoff, white paper, reserve composition, reserve custody, redemption, recovery plan, redemption plan, and no-raw-customer-record handoff. The Jurisdiction Checklist also shows a MiCA ART/EMT stablecoin issuer control that becomes evidence-ready only from metadata-only records carrying `control-eu-mica-art-emt-stablecoin-issuer-regime` plus matching asset-referenced-token, asset-referenced-token-classification, e-money-token, electronic-money-token, ART-issuer, EMT-issuer, MiCA-stablecoin, stablecoin-issuer, home-Member-State, competent-authority, authorisation, notification-route, white-paper, public-offer, admission-to-trading, reserve-composition, reserve-segregation, reserve-custody, liquidity-management, redemption-rights, par-value-redemption, recovery-plan, or redemption-plan evidence text, so source-free stablecoin memos remain needs-evidence. Not legal advice.

EU MiCA Title II crypto-asset launch projects now show a Jurisdiction Checklist white-paper and public-communication control that becomes evidence-ready only from metadata-only source-linked RWA disclosure and evidence-anchor records carrying `control-eu-mica-title-ii-white-paper`, including white paper, public communication, risk disclosure, management approval, manifest provenance, source lineage, and reviewer-owner evidence. Not legal advice.

![EU MiCA ART/EMT stablecoin source graph](docs/assets/screenshots/eu-mica-stablecoin-issuer-source-graph.png)

UK qualifying stablecoin issuer projects now route to FCA PS26/10 and BoE/FCA joint-approach source controls, with evidence gaps for UKQS issuer route, regulated activity assumptions, admission or distribution scope, disclosures, backing assets, safeguarding, reconciliation, redemption, liquidity, recordkeeping, systemic-transition handoff, and no-raw-customer-record evidence. The Jurisdiction Checklist also shows a UK qualifying stablecoin issuer control that becomes evidence-ready only from metadata-only records carrying `control-uk-fca-qualifying-stablecoin-issuer-regime` plus matching UK-qualifying-stablecoin, UKQS-issuer, UK-stablecoin-issuer, regulated-activity, admission-to-trading, distribution-scope, disclosure-owner, governance-owner, FCA-BoE-handoff, stablecoin-backing-assets, backing-asset, safeguarding, reconciliation, redemption, liquidity, recordkeeping, systemic-transition, or joint-regulation evidence text, so source-free stablecoin memos remain needs-evidence. Not legal advice.

![UK qualifying stablecoin issuer source graph](docs/assets/screenshots/uk-stablecoin-issuer-source-graph.png)

Singapore DPT custody projects now route to MAS PSN02 AML/CFT controls and MAS PS-G03 customer-asset safeguard controls, with evidence gaps for AML/CFT risk assessment, CDD, sanctions, transaction monitoring, data-redaction/model handoff, segregation, custody disclosure, reconciliation, transfer controls, and local-counsel handoff. The Jurisdiction Checklist also shows separate MAS PSN02 DPT AML/CFT and MAS PS-G03 customer-asset safeguard controls that become evidence-ready only from metadata-only records carrying `control-sg-mas-psn02-dpt-aml-cft` or `control-sg-mas-dpt-customer-asset-safeguards` plus matching MAS-PSN02, DPT-AML, AML/CFT-risk-assessment, CDD, sanctions, transaction-monitoring, wallet-history-metadata-boundary, model-handoff, MAS-PS-G03, customer-asset-segregation, safeguarding, wallet-authority, signer-quorum, transfer-control, custody-disclosure, reconciliation, withdrawal-approval, emergency-pause, incident-response, customer-asset-return, or custody-and-signer-control evidence text, so source-free DPT memos remain needs-evidence. No raw KYC, customer records, wallet secrets, or legal conclusions. Not legal advice.

![Singapore DPT custody source graph](docs/assets/screenshots/singapore-dpt-custody-source-graph.png)

Hong Kong VATP custody projects now route to SFC VATP client-asset custody controls, with evidence gaps for associated-entity trust holding, wallet segregation, reconciliation, cold-storage/key-control, monitoring, compensation arrangements, and local-counsel handoff. The Jurisdiction Checklist also shows VATP client-asset custody and wallet-governance/compensation controls that become evidence-ready only from metadata-only records carrying `control-hk-sfc-vatp-client-asset-custody` plus matching SFC-VATP, client-asset, associated-entity, segregation, safeguarding, wallet-authority, cold-storage, key-management, reconciliation, internal-audit, monitoring, incident-response, compensation-arrangement, or custody-and-signer-control evidence text, so source-free custody memos remain needs-evidence. No raw KYC, customer records, wallet secrets, private cryptographic material, personal data, or legal conclusions. Not legal advice.

![Hong Kong VATP custody source graph](docs/assets/screenshots/hong-kong-vatp-source-graph.png)

Hong Kong stablecoin issuer projects now route to HKMA Stablecoins Ordinance source controls, with evidence gaps for licence/application scope, regulated stablecoin activity, reserve assets, full backing, redemption, attestation, AML/CFT, recordkeeping, user-protection, and local-counsel handoff. The Jurisdiction Checklist also shows the HKMA stablecoin issuer licensing, reserve, and AML/CFT control that becomes evidence-ready only from metadata-only records carrying `control-hk-hkma-stablecoin-issuer-regime` plus matching Hong-Kong-stablecoin-issuer, fiat-referenced-stablecoin, Stablecoins-Ordinance, HKMA-licence, regulated-stablecoin-activity, reserve-assets, full-backing, redemption, AML-CFT, CDD, blockchain-analytics, suspicious-transaction, or record-keeping evidence text, so source-free stablecoin memos remain needs-evidence. No raw KYC, customer records, credentials, wallet secrets, private cryptographic material, personal data, or legal conclusions. Not legal advice.

![Hong Kong HKMA stablecoin issuer source graph](docs/assets/screenshots/hong-kong-stablecoin-source-graph.png)

Hong Kong tokenised product projects now route to SFC tokenisation and secondary-trading circular source controls, with evidence gaps for SFC-authorised product scope, prior consultation or approval route, ownership recordkeeping, operational soundness, cybersecurity/data-privacy/business-continuity controls, smart-contract integrity, price-deviation alert, indicative NAV, market-maker and liquidity arrangements, trading suspension workflow, investor risk disclosure, notification, and local-counsel handoff. The Jurisdiction Checklist also shows an SFC tokenised product authorisation and secondary-trading control that becomes evidence-ready only from metadata-only records carrying `control-hk-sfc-tokenised-investment-products-secondary-trading` plus matching SFC-authorised-product, tokenised-product, product-authorisation, prior-consultation, prior-approval, ownership-records, operational-soundness, smart-contract-integrity, secondary-trading, price-deviation-alert, indicative-NAV, market-maker, liquidity, trading-suspension, client-confirmation, or notification-workflow evidence text, so source-free tokenised product memos remain needs-evidence. No raw investor records, wallet secrets, credentials, personal data, or legal conclusions. Not legal advice.

![Hong Kong SFC tokenised product source graph](docs/assets/screenshots/hong-kong-tokenised-product-source-graph.png)

Japan crypto custody projects now route to FSA crypto-asset exchange supervision and user-asset protection controls, with evidence gaps for registration assumptions, user-asset protection, information to users, cold-wallet/offline management, reconciliation, leakage response, and local-counsel handoff. The Jurisdiction Checklist also shows FSA registration/user-asset protection and cold-wallet/leakage-response controls that become evidence-ready only from metadata-only records carrying `control-jp-fsa-crypto-asset-custody-user-protection` plus matching Japan-crypto-asset-exchange, FSA-registration, user-asset-protection, information-to-users, contract-details, cold-wallet, offline-environment, segregated-wallet, daily-reconciliation, leakage-response, separate-management-audit, or custody-and-signer-control evidence text, so source-free Japan custody memos remain needs-evidence. No raw KYC, customer records, wallet secrets, personal data, or legal conclusions. Not legal advice.

![Japan crypto custody source graph](docs/assets/screenshots/japan-crypto-custody-source-graph.png)

Canada CTP custody projects now route to CSA Staff Notice 21-332 / 21-329 controls, with evidence gaps for registration and PRU assumptions, Canadian client access, no leverage, VRCA consent, client-asset custody and segregation, acceptable third-party custodian assurance, no re-hypothecation, insurance or alternative risk mitigation, and local-counsel handoff. The Jurisdiction Checklist also shows CSA registration/PRU investor-protection and client-asset custody/segregation controls that become evidence-ready only from metadata-only records carrying `control-ca-csa-ctp-pru-custody-investor-protection` plus matching Canada-CSA-PRU, pre-registration-undertaking, registration-application, Canadian-client, no-leverage, value-referenced-crypto-asset, prior-written-consent, acceptable-third-party-custodian, 80%-custodian, trust, separate-and-apart, designated-trust-account, re-hypothecation, SOC-2, insurance-risk-mitigation, or custody-and-signer-control evidence text, so source-free Canada custody memos remain needs-evidence. No raw KYC, raw client records, wallet secrets, personal data, or legal conclusions. Not legal advice.

![Canada CTP custody source graph](docs/assets/screenshots/canada-ctp-custody-source-graph.png)

Australia digital asset projects now route to ASIC INFO 225, ASIC RG 133, and AUSTRAC virtual asset service guidance, with evidence gaps for digital-asset financial-services scope, AFS licensing handoff, dealing/market-making/custodial-service assumptions, crypto custody controls, VASP AML/CTF programs, CDD, travel rule, reporting, seven-year recordkeeping, and local-counsel handoff. The Jurisdiction Checklist also shows ASIC digital-asset financial-services/custody and AUSTRAC VASP AML/CTF controls that become evidence-ready only from metadata-only records carrying `control-au-asic-austrac-digital-asset-financial-services` plus matching Australia-ASIC-digital-asset, AFS-licence, financial-product, custodial-depository-service, client-assets-separate, cold-storage, independent-audit, AUSTRAC, virtual-asset-service-provider, AML/CTF-program, customer-due-diligence, travel-rule, suspicious-matter-report, threshold-transaction-report, seven-years, or custody-and-signer-control evidence text, so source-free Australia ASIC/AUSTRAC memos remain needs-evidence. No raw KYC, raw customer records, wallet secrets, personal data, or legal conclusions. Not legal advice.

![Australia digital asset source graph](docs/assets/screenshots/australia-digital-asset-source-graph.png)

Korea VASP user protection projects now route to FSC Virtual Asset User Protection Act / Enforcement Decree and KoFIU VASP reporting / AML guidance, with evidence gaps for user deposits at banks, user virtual-asset segregation, 80 percent cold-wallet handling, insurance/reserve, abnormal trading monitoring/reporting, user disclosure, CDD/EDD, STR, Travel Rule, real-name account, ISMS, and local-counsel handoff. The Jurisdiction Checklist also shows FSC user-asset protection/custody and KoFIU VASP reporting/AML controls that become evidence-ready only from metadata-only records carrying `control-kr-fsc-kofiu-vasp-user-protection-aml` plus matching Korea-VASP-user-protection, user-deposits-at-banks, users-virtual-assets-separate, 80-percent-cold-wallet, cold-wallet, insurance-reserve, abnormal-trading-monitoring, Korean-language-whitepaper, KoFIU, VASP-reporting, compliance-system, major-shareholders, ISMS, real-name-verified-checking-account, customer-due-diligence, suspicious-transaction-report, travel-rule, or custody-and-signer-control evidence text, so source-free Korea VASP custody and AML memos remain needs-evidence. No raw KYC, customer records, identity files, wallet secrets, personal data, or legal conclusions. Not legal advice.

![Korea VASP user protection source graph](docs/assets/screenshots/korea-vasp-user-protection-source-graph.png)

India VDA projects now route to the Ministry of Finance PMLA VDA notification and FIU-IND AML/CFT guidance, with evidence gaps for VDA activity scope, FIU-IND Reporting Entity registration, AML/CFT/CPF program, Designated Director / Principal Officer, CDD/EDD, beneficial ownership, STR/monthly reporting, transaction monitoring, risk assessment, Travel Rule, record retention, FINGate/reporting-format handoff, and local-counsel review. The Jurisdiction Checklist also shows FIU-IND Reporting Entity registration/activity-scope and India VDA AML/CFT reporting, CDD/EDD, STR, and Travel Rule controls that become evidence-ready only from metadata-only records carrying `control-in-fiu-pmla-vda-aml-cft` plus matching India-VDA-service-provider, India-FIU-IND-registration, India-PMLA-reporting-entity, Designated-Director, Principal-Officer, client-money-account, AML/CFT/CPF-program, board-senior-management, India-VDA-SP-activity-scope, VDA-activity-scope, India-VDA-AML-CFT, FIU-IND-reporting, India-suspicious-transaction-report, India-Travel-Rule, India-transaction-monitoring, India-risk-assessment, FINGate-VASP-reporting, Ground-of-Suspicion, India-record-retention, India-no-anonymous-wallet, India-beneficial-ownership, or custody-and-signer-control evidence text, so source-free India VDA PMLA memos remain needs-evidence. No raw KYC, PAN, Aadhaar, OVDs, customer records, wallet secrets, personal data, or legal conclusions. Not legal advice.

![India VDA PMLA source graph](docs/assets/screenshots/india-vda-pmla-source-graph.png)

Thailand digital asset business projects now route to SEC digital asset exchange, broker, dealer, custodial wallet provider, licensing, client-asset custody, transfer-approval, daily-reconciliation, and AMLO AML/CDD source controls, with evidence gaps for high-risk customer, beneficial-owner, internal-control, training, reporting-owner, and no-raw-KYC handoff review. The Jurisdiction Checklist also shows SEC digital asset business license/client-asset custody and AMLO AML/CDD high-risk customer controls that become evidence-ready only from metadata-only records carrying `control-th-sec-digital-asset-business-custody-aml` plus matching Thailand-digital-asset-business, digital-asset-exchange, digital-asset-broker, digital-asset-dealer, custodial-wallet-provider, SEC-license-route, client-asset-records, daily-reconciliation, transfer-approval, client-asset-use-prohibition, AMLO, CDD-EDD, beneficial-ownership, high-risk-customer, customer-identification, internal-control, training, reporting-owner, or custody-and-signer-control evidence text, so source-free Thailand SEC/AMLO custody memos remain needs-evidence. No raw KYC, customer records, wallet secrets, personal data, or legal conclusions. Not legal advice.

Indonesia digital financial asset and crypto asset trading projects now route to OJK licensing, PAKD/CPAKD whitelist, SPRINT, trading activity, product/instrument registration, reporting, business-plan, main-party competence/compliance assessment, governance, integrity, consumer-protection, and no-raw-KYC source controls. The Jurisdiction Checklist also shows OJK digital financial asset trading licensing/whitelist and product, reporting, governance, and main-party controls that become evidence-ready only from metadata-only records carrying `control-id-ojk-digital-financial-asset-crypto-trading` plus matching Indonesia-digital-financial-asset-trading, Indonesia-crypto-asset-trading, OJK, PAKD, CPAKD, whitelist, SPRINT-licensing-route, licensed-registered-operator, official-app-and-website, consumer-protection, POJK-27, POJK-23, SEOJK-20, product-registration, instrument-registration, daily-report, monthly-report, business-plan, main-parties, competence, compliance-assessment, governance, or integrity evidence text, so source-free Indonesia OJK crypto trading memos remain needs-evidence. No raw KYC, customer records, wallet secrets, personal data, or legal conclusions. Not legal advice.

Malaysia digital asset exchange projects now route to SC Malaysia Digital Assets / Guidelines on Digital Assets and BNM AML/CFT digital-currency source controls, with evidence gaps for RMO-DAX, digital broker, Digital Asset Custodian, IEO, tradeable-asset, official-channel, reporting-institution, STR, recordkeeping, transparency, and no-raw-KYC handoff review. The Jurisdiction Checklist also shows SC DAX/DAC registration, trading, and custody plus BNM digital currency AML/CFT reporting-institution controls that become evidence-ready only from metadata-only records carrying `control-my-sc-bnm-digital-asset-exchange-custody-aml` plus matching Malaysia-digital-asset, Securities-Commission-Malaysia, SC-Malaysia, RMO-DAX, DAX-operator, digital-asset-exchange, digital-broker, Digital-Asset-Custodian, DAC, IEO, tradeable-asset, official-channels, custody-safeguarding, Bank-Negara-Malaysia, BNM, digital-currency-exchanger, reporting-institution, AML-CFT, customer-identification, CDD-EDD, beneficial-ownership, suspicious-transaction-report, STR, compliance-officer, recordkeeping, or transparency evidence text, so source-free Malaysia SC/BNM digital asset memos remain needs-evidence. No raw KYC, customer records, wallet secrets, personal data, or legal conclusions. Not legal advice.

![Malaysia digital asset exchange source graph](docs/assets/screenshots/malaysia-digital-asset-exchange-source-graph.png)

Philippines VASP custody projects now route to BSP Circular No. 1108 and BSP Memorandum No. M-2026-003 source controls, with evidence gaps for VASP/CASP counterparty status, BSP registration or Certificate of Authority, VA exchange, transfer, safekeeping, VA custodian, wallet security, retail/offshore access, due diligence/EDD, transaction monitoring, STR, recordkeeping, staff training, and no-raw-KYC handoff review. The Jurisdiction Checklist also shows BSP VASP registration, activity, and custody-scope plus BSP AML/CFT due-diligence, monitoring, and STR controls that become evidence-ready only from metadata-only records carrying `control-ph-bsp-vasp-casp-risk-management-aml` plus matching Philippines-VASP, Philippine-VASP, Bangko-Sentral, BSP, Certificate-of-Authority, money-service-business, virtual-asset-service-provider, crypto-asset-service-provider, CASP, VA-exchange, VA-transfer, VA-custodian, safekeeping, wallet-security, retail-access, Philippines-AML-CFT, BSP-AML, risk-assessment, due-diligence, EDD, proof-of-registration, adverse-media, FATF-Recommendation-16, payment-transparency, transaction-monitoring, suspicious-transaction-report, STR, recordkeeping, or staff-training evidence text, so source-free Philippines BSP VASP memos remain needs-evidence. No raw KYC, customer records, wallet secrets, personal data, or legal conclusions. Not legal advice.

![Philippines VASP custody source graph](docs/assets/screenshots/philippines-vasp-custody-source-graph.png)

South Africa CASP projects now route to FSCA crypto-asset financial-product and FIC Directive 9 Travel Rule source controls, with evidence gaps for CASP/FSP licence route, advice/intermediary/investment-management scope, business model, operational ability, fit-and-proper owner, ordering/intermediary/recipient CASP role mapping, counterparty due diligence, secure transmission, incomplete-transfer handling, unhosted-wallet policy, RMCP, recordkeeping, and no-raw-KYC handoff review. The Jurisdiction Checklist also shows FSCA CASP/FSP licensing/activity-scope and FIC Travel Rule/RMCP/transfer-control controls that become evidence-ready only from metadata-only records carrying `control-za-fsca-fic-casp-licensing-travel-rule` plus matching South-Africa-CASP, South-African-CASP, ZA-CASP, FSCA, crypto-asset-service-provider, financial-product, FAIS, FSP-licence, FSP-license, financial-services-provider, advice, intermediary, investment-management, business-model, operational-ability, fit-and-proper, FIC-Directive-9, South-Africa-Travel-Rule, Travel-Rule, ordering/intermediary/recipient-crypto-asset-service-provider, originator, beneficiary, counterparty-CASP, due-diligence, secure-transmission, recordkeeping, Risk-Management-and-Compliance-Programme, RMCP, unhosted-wallet, or no-raw-KYC evidence text, so source-free South Africa CASP Travel Rule memos remain needs-evidence. No raw KYC, customer records, wallet secrets, originator data, beneficiary data, personal data, or legal conclusions. Not legal advice.

![South Africa CASP Travel Rule source graph](docs/assets/screenshots/south-africa-casp-travel-rule-source-graph.png)

UK cryptoasset AML projects now route to FCA AML/CTF regime, MLR registration expectations, and Travel Rule expectations, with evidence gaps for cryptoasset exchange/custodian-wallet activity scope, FCA MLR registration, business plan, ownership/control, MLRO, BWRA/CRA, AML/CTF/CPF framework, CDD/EDD, sanctions, blockchain analytics, transaction monitoring, SAR escalation, Travel Rule originator/beneficiary data flow, record retrieval, and local-counsel handoff. The Jurisdiction Checklist also shows FCA MLR registration/activity-scope and UK cryptoasset AML, SAR, sanctions, and Travel Rule controls that become evidence-ready only from metadata-only records carrying `control-uk-fca-cryptoasset-aml-registration-travel-rule` plus matching UK-cryptoasset-business, UK-FCA-MLR-registration, UK-cryptoasset-exchange-provider, UK-custodian-wallet-provider, UK-business-plan, UK-ownership-control-structure, UK-MLRO, UK-business-wide-risk-assessment, UK-customer-risk-assessment, UK-AML-framework, UK-cryptoasset-Travel-Rule, UK-originator-beneficiary-information, UK-suspicious-activity-reporting, UK-sanctions-screening, UK-blockchain-analytics, UK-transaction-monitoring, UK-CDD-EDD, UK-Travel-Rule-data-flow, UK-third-party-tool-configuration, or UK-record-retrieval evidence text, so source-free UK cryptoasset AML memos remain needs-evidence. No raw KYC, customer records, wallet secrets, private cryptographic material, personal data, or legal conclusions. Not legal advice.

![UK cryptoasset AML source graph](docs/assets/screenshots/uk-cryptoasset-aml-source-graph.png)

Germany MiCAR custody projects now route to BaFin/MiCAR CASP service-scope, Article 60 notification or Article 62 authorisation assumptions, Article 75 custody safeguarding, client-position records, segregation, return-process, means-of-access handling, private-key exclusion, and German local-counsel handoff evidence gaps. The Jurisdiction Checklist also shows BaFin MiCAR CASP authorisation/notification and MiCAR custody safeguarding/client-position controls that become evidence-ready only from metadata-only records carrying `control-de-bafin-micar-casp-custody-authorisation` plus matching Germany-MiCAR, CASP-authorisation, Article-60, Article-62, notification, home-Member-State, German-client-access, Germany-service-scope, Article-75, custody-policy, client-register, position-statement, segregation, return-crypto-assets, means-of-access, private-cryptographic-keys, or client-crypto-assets evidence text, so source-free Germany MiCAR custody memos and source-only shared custody runbooks remain needs-evidence. No raw KYC, customer records, wallet secrets, private cryptographic material, personal data, or legal conclusions. Not legal advice.

![Germany MiCAR custody source graph](docs/assets/screenshots/germany-micar-custody-source-graph.png)

UAE VARA operating projects now route to the VARA Virtual Assets and Related Activities Regulations 2023 and VARA Compliance and Risk Management Rulebook, with evidence gaps for virtual-asset activity scope, licensing assumptions, compliance management, AML/CFT, books and records, audit, client virtual asset custody, proof-of-reserves, reconciliation, and UAE local-counsel handoff. The Jurisdiction Checklist also shows VARA virtual-asset activity-scope and compliance/risk-management custody-access controls that become evidence-ready only from metadata-only records carrying `control-uae-vara-va-regulations-activity-scope` or `control-uae-vara-compliance-risk-management`, without raw KYC, customer records, wallet secrets, private cryptographic material, personal data, or legal conclusions. The separate VARA 2024 marketing controls still require marketing-specific trigger facts so operating/custody reviews do not inherit promotion-only gaps. Not legal advice.

![UAE VARA operating source graph](docs/assets/screenshots/uae-vara-operating-source-graph.png)

The marketing claims scenario routes `SignalBridge Marketing Review` into US FTC endorsement/advertising, EU MiCA marketing communications, UK FCA financial-promotion, and UAE VARA 2024 Marketing Regulations controls, including communication labels, white-paper consistency, home/host Member State audience routing, promotion approval, risk-warning, KOL/incentive, and recordkeeping evidence gaps. The Jurisdiction Checklist also shows US FTC advertising claims, US SEC investment adviser marketing, EU MiCA marketing communications, UK FCA financial-promotion, and UAE VARA 2024 marketing controls that become evidence-ready from metadata-only claims-substantiation, creator-disclosure, adviser advertisement, promoter, conflict-disclosure, Form ADV, performance, books-and-records, white-paper-consistency, Member State audience, notification, publication-timing, approval-pack, clear-and-balanced, client-categorisation, retail-access, VARA approval, promotional-label, misleading-claim, KOL, incentive, and recordkeeping marketing template records. UK financial-promotion approval controls become evidence-ready only from metadata-only records carrying `control-uk-fca-crypto-financial-promotions` plus matching promotion, approval, or eligibility evidence text. UAE VARA marketing approval/audience and KOL/incentive controls become evidence-ready only from metadata-only records carrying `control-uae-vara-marketing-regulations-2024` plus their matching approval-route or KOL/recordkeeping evidence text, without raw customer records, credentials, wallet secrets, private cryptographic material, personal data, or legal conclusions. When project facts mention investment adviser, private-fund, Form ADV, testimonial, endorsement, or performance-presentation scope, the US SEC Investment Adviser Marketing Rule source control routes those advertisement, promoter, performance, and books-and-records evidence requests without making legal conclusions. Not legal advice.

The US FTC advertising claims and endorsement disclosure control now becomes evidence-ready only from metadata-only marketing records carrying `control-us-ftc-endorsement-advertising-guides` plus matching advertising-claim, claim-inventory, claims-substantiation, risk-disclosure, campaign-channel, endorsement, testimonial, material-connection, creator-disclosure, influencer, approval-routing, or monitoring-owner evidence text, so source-free marketing memos remain needs-evidence. Not legal advice.
...
![Workspace Action Queue handoff recovery](docs/assets/screenshots/workspace-action-handoff-recovery.png)
...
The Source Review Ledger has a source review as-of date field so reviewers can test planned counsel handoff dates and surface due source update approvals before export. It can sync metadata-only source review records to the Phase 2 API and refresh/download a server Source Review Packet from `GET /api/workspaces/:workspaceId/source-reviews/packet` with ledger hashes, review counts, packet hash, validated non-empty recovery next actions, and no reviewer-note body text. The Source Update Approval Queue can sync metadata-only approval records to the Phase 2 API, refresh persisted records back into the command center through `GET /api/workspaces/:workspaceId/source-approvals`, and refresh/download a server Source Approval Packet from `GET /api/workspaces/:workspaceId/source-approvals/packet` with queue hashes, approval-gate counts, packet hash, validated non-empty recovery next actions, and no reviewer-note body text. Matching behavior stays unchanged until counsel or compliance review records refreshed source metadata. Not legal advice.
...
Evidence Ledger records local evidence creation, template application, source-gap requests, duplicate-safe request refreshes, edits, and removals as audit-prep events with a standalone JSON export. Manual draft intake now previews blocked or review-needed data classes with redacted recovery guidance before downstream vault/model/export handoff. Local file intake checks filename and file metadata before hashing; raw KYC, credentials, and private-key-like metadata are blocked with redacted recovery guidance, while raw file bytes remain local. When a new project has no evidence, Evidence Intake Guidance recommends the safest next template and concrete metadata-only requests from the current risk coverage. Not legal advice.

![Evidence Draft Boundary Preview](docs/assets/screenshots/evidence-draft-boundary-preview.png)
...
Integration Enablement Dossier consolidates the registry plus model provider, secret handling, object storage, document parser, chain anchor, and GRC destination policy reports into one hashed JSON handoff. The nested Integration Enablement Gate turns adapter blockers, policy gaps, and missing or blocked server receipts into a hashed recovery queue with its own JSON download, so reviewers can see the next enablement blocker without enabling providers, storage, OCR, ticket systems, or chains. It remains metadata-only and keeps all external adapters disabled until a separate enablement review. Not legal advice.

![Integration Enablement Dossier](docs/assets/screenshots/integration-enablement-dossier.png)

![Integration Enablement Gate](docs/assets/screenshots/integration-enablement-gate.png)

![Integration Policy Receipt Coverage](docs/assets/screenshots/integration-policy-receipt-coverage.png)

Policy Evaluation Receipts show the latest server-side integration policy evaluations for the workspace, including policy ID, control readiness, report hash, context hash, and disabled external capability state. The workbench can refresh persisted receipts from `GET /api/workspaces/:workspaceId/integration-policy-evaluations` after a reload or handoff, download a metadata-only receipt bundle, and refresh `GET /api/workspaces/:workspaceId/integration-policy-evaluations/recovery` before downloading a receipt recovery packet with a stable packet hash, missing/blocked/needs-policy/stale counts, validated non-empty next actions, and `externalEnablementAllowed: false` for adapter enablement review. They are audit preparation metadata only and do not store credentials, raw evidence, raw KYC, personal data, or external write commands. Not legal advice.

![Integration Policy Evaluation Receipts](docs/assets/screenshots/integration-policy-receipts-refresh.png)

Model Gateway Provider Policy makes disabled external model adapters and required controls visible before any server-side provider proxy is enabled. The workbench can refresh the metadata-only policy from `GET /api/model-gateway/provider-policy`, after Model Connect validation use `POST /api/model-gateway/provider-policy` with only provider, mode, status, and blocker metadata, and evaluate `POST /api/model-gateway/secret-policy` from a metadata-only secret policy draft. It never sends session API keys, endpoint hosts, model names, provider labels, raw evidence, or provider credentials to policy routes. The panel shows server sync, validates non-empty recovery next actions, exports metadata-only JSON, and keeps external providers disabled until secret policy, provider allowlist, egress logging, Redaction Gate, human-review controls, and a separate adapter enablement review are approved. Not legal advice.
...
Object Storage Policy Evaluation turns Evidence Retention, manifest availability, retention/deletion windows, encryption, bucket allowlist, access logging, lifecycle controls, sensitive-material confirmation, and Human Review enforcement into a server-verifiable readiness report with validated non-empty recovery next actions. It sends only policy metadata and context, never raw evidence bytes or credentials, and keeps external object storage disabled until a separate adapter enablement review. Not legal advice.

![Object Storage Policy Evaluation](docs/assets/screenshots/object-storage-policy-evaluation.png)

Document Parser Policy Evaluation turns Evidence Retention, Export Safety, manifest availability, document-size limits, raw-document retention, deletion SLA, parser purpose, redaction-before-parsing, no-training-use, access logging, sensitive-material confirmation, and Human Review enforcement into a server-verifiable readiness report with validated non-empty recovery next actions. It sends only whitelisted policy metadata and context, never raw document bytes, raw document body, credentials, or final legal-decision requests, and keeps external document parsing disabled until a separate raw-document adapter enablement review. Not legal advice.

![Document Parser Policy Evaluation](docs/assets/screenshots/document-parser-policy-evaluation.png)

Chain Anchor Policy Evaluation turns manifest availability, Counsel Pack version readiness, network scope, wallet custody notes, signer role, transaction logging, privacy review, public payload limits, user consent, and Human Review enforcement into a server-verifiable readiness report with validated non-empty recovery next actions. It sends only policy metadata and context, never wallet keys, signed transactions, raw evidence, KYC, credentials, or real chain-write instructions, and keeps external chain anchoring disabled in `simulated-only` mode until a separate wallet signing and transaction enablement review. Not legal advice.

![Chain Anchor Policy Evaluation](docs/assets/screenshots/chain-anchor-policy-evaluation.png)

GRC Destination Policy Evaluation turns the remediation queue, Export Safety Gate, local GRC ticket export readiness, destination scope, field mapping, authentication policy, redaction, ticket ownership, retry/audit logging, and Human Review enforcement into a server-verifiable readiness report with validated non-empty recovery next actions. It sends only whitelisted policy metadata and context, never API keys, webhook secrets, raw ticket bodies, raw KYC, personal data, or external ticket-write commands, and keeps external GRC ticket creation disabled until a separate adapter enablement review. Not legal advice.
...
Evidence Vault sync carries linked regulatory control IDs and redacted metadata-boundary warnings from ledger source references into server records and the manifest hash while keeping raw evidence bytes and raw identifiers out of API responses. After sync, users can download the server-side **Vault Manifest JSON** as a metadata-only handoff artifact, inspect the **Evidence Vault Lineage Digest** for active/replaced/rejected counts, manifest hash, lineage links, linked controls, and download a hashed lineage JSON before counsel handoff. The workbench can also refresh the server **Evidence Vault Lineage Recovery Packet** from `GET /api/workspaces/:workspaceId/evidence-lineage-recovery`, then download open rejected evidence IDs, missing-manifest recovery state, packet hash, and non-empty next actions without source-note body text. The ledger also shows **Evidence Vault Control Coverage** so reviewers can see which controls are linked across vault records and manifest items. Not legal advice.
...
Manifest Drift Guard compares the current Evidence Manifest against refreshed Evidence Vault local-content hash lineage, the latest saved Counsel Pack version, and the latest server export record. If evidence changes after a version or export is saved, the workbench shows the stale target, recovery action, report hash, and a metadata-only JSON download before counsel or judge handoff. Not legal advice.

![Manifest Drift Guard stale version](docs/assets/screenshots/manifest-drift-guard-stale-version.png)

Human Review records reviewer due dates, saved status history, audit log IDs, source clause-match refresh actions, and a downloadable review timeline. Returned or rejected items also produce a Human Review Recovery Packet JSON with a stable packet hash, redacted decision notes, target-specific recovery actions, and a Not legal advice boundary before handoff. The workbench can refresh the same server packet directly from `GET /api/workspaces/:workspaceId/reviews/recovery`, download the metadata-only JSON, and let demo smoke checks validate packet hashes plus non-empty recovery actions without returning raw comments.
...
The Source Review Ledger can also export a metadata-only Source Review Packet JSON with a packet hash, source records, and clause-match refresh actions for counsel/compliance review. When the Phase 2 API is running, the same command-center section can refresh/download the server Source Review Packet with ledger hashes, review counts, a packet hash, and no reviewer-note body text. Not legal advice.

![Source Review Packet](docs/assets/screenshots/source-review-packet.png)

The Source Review Ledger can sync metadata-only source review records to the Phase 2 API with a stable ledger hash and refresh the server Source Review Packet while keeping source matching unchanged.
...
Server Model Run Ledger refreshes persisted Model Gateway run summaries from `GET /api/workspaces/:workspaceId/model-runs`, showing run status, human-review status, retry state, and receipt hashes after the Secure Review Journey completes. It can download a metadata-only Model Run Receipt JSON with a stable receipt hash, provider policy metadata, remediation steps, and no raw prompt, response, evidence body, or credentials. The Phase 2 API also exposes `GET /api/workspaces/:workspaceId/model-runs/recovery`, returning a workspace-level metadata-only recovery packet with blocked, retry-available, human-review-needed, and ready counts plus validated non-empty next actions and redacted per-run recovery actions. Not legal advice.
...
Server Audit Log Explorer turns Secure Review Journey audit-log records into a metadata-only timeline with actors, action counts, targets, before/after hashes, per-event entry hashes, a stable export hash, an integrity chain hash, JSON download, and Phase 2 API refresh filters for actor, action, target type, and target ID. The Phase 2 API also exposes a filtered Audit Log export route plus `GET /api/workspaces/:workspaceId/audit-log/recovery` with redacted event hashes, integrity chain metadata, recovery remediation, validated non-empty next actions for empty/blocked/review-needed exports, packet hashes, and the Not legal advice boundary; the Audit Log client validates both artifacts before any UI or workflow trusts them. Not legal advice.
...
Counsel Pack exports template-specific Markdown, browser Print / Save PDF output, Model Intake summary, AI event hashes, source review freshness ledger, source update approval gates, manifest JSON, version-history JSON, standalone version diff JSON, and a simulated anchor receipt without claiming a real chain write. Model Intake can also download its own profile, event ledger, readiness checklist, and event hashes as JSON.
...
Counsel Pack version history records the manifest hash, Markdown hash, review-status snapshot, source snapshot, Regulatory Source Pack hash, source review status, export timestamp, and diff from the previous export without storing raw Markdown content in the version JSON. Versions with a previous export can also download a standalone Diff JSON artifact for counsel or judge comparison.

![Counsel Pack version history](docs/assets/screenshots/counsel-pack-version-history.png)

![Counsel Pack diff download](docs/assets/screenshots/counsel-pack-diff-download.png)

Server export records let the demo create a Phase 2 API-backed Counsel Pack handoff from the latest saved Pack Version, including the Regulatory Source Pack hash, source review status, and Jurisdiction Readiness Digest hash/status/handoff flag. The server record is metadata-only and repeats the Not legal advice boundary. Each row can also download a server export receipt JSON that carries only record metadata, manifest/artifact/source-pack hashes, jurisdiction readiness metadata, review summary, recovery action, stable receipt hash, and the Not legal advice boundary. The same panel can refresh the server export recovery packet from `GET /api/workspaces/:workspaceId/exports/counsel-pack/recovery` before downloading the metadata-only packet JSON.

![Counsel Pack server export record](docs/assets/screenshots/counsel-pack-server-export-record.png)

![Counsel Pack server export receipt download](docs/assets/screenshots/counsel-pack-server-export-receipt-download.png)

The Counsel Handoff Checklist condenses Export Safety Inventory, Evidence Manifest, Manifest Drift Guard, Evidence Recertification Queue, Evidence Vault Lineage Digest, Regulatory Source Pack, Counsel Review Status, Counsel Pack Version, server export record, and Submission Pack readiness into one downloadable metadata-only handoff JSON with a stable checklist hash. Not legal advice.
...
Sources now includes generated Submission Pack JSON and Demo Runbook JSON artifacts for judges. The Submission Pack summarizes demo readiness, Demo Runbook hash, export safety summary, required assets, feature-to-theme mapping, known limitations, manifest hash, Regulatory Source Pack hash, Regulatory Source Coverage hash/status, open source evidence count, and the Not legal advice boundary.
...
Sources also includes an Export Safety Inventory that consolidates the handoff state for manifest, Manifest Drift Guard, Evidence Vault Lineage Digest, Evidence Disposal Runbook, source pack, Counsel Pack version, source review, Source Update Approval Queue, Model Gateway Evaluation, Source Freshness Board, Demo Runbook, local counsel routing, GRC ticket export, integration dossier, Integration Enablement Gate, and submission pack artifacts. It carries the Manifest Drift Guard, Model Gateway Evaluation, Evidence Vault Lineage Digest, Evidence Disposal Runbook, Integration Enablement Gate, Source Freshness Board, Source Update Approval Queue, and Demo Runbook artifact hashes/statuses when available, blocks external handoff when the data boundary detects private keys, credential-like tokens, or raw KYC references, and downloads only redacted metadata. Not legal advice.
...
![Export Safety Inventory Source Update Approval Queue](docs/assets/screenshots/export-safety-source-approval-queue.png)

![Export Safety Inventory Model Gateway Evaluation](docs/assets/screenshots/export-safety-model-gateway-evaluation.jpg)
...
9. Run **Secure Review Journey** against the Phase 2 API to create a backend workspace, sync metadata-only evidence, create a server Model Gateway receipt, read the automatically queued Human Review request, fetch audit log records, refresh the persisted server Model Run Ledger, and download the Model Run Receipt JSON when a reviewer needs the metadata-only run hash. The server request is limited to audit-prep metadata, evidence hashes, and risk flag summaries.
...
9. Add or edit records in **Evidence Ledger**, move local evidence through `draft`, `requested`, `received`, `under-review`, `verified`, and `rejected` workflow states, hash a local file into metadata-only evidence, use Evidence Intake Guidance when the ledger is empty, request missing evidence from Risk Audit, request source evidence gaps from the Command Center without creating duplicate Ledger rows, or apply one of the scenario templates for tokenized yield/RWA, DAO governance/multisig, or AI compliance workflows. Source references can include `source gap: ...`, `regulatory control: control-...`, or `regulatory clause: ...` so Evidence Vault sync carries linked control IDs, redacted metadata-boundary warnings, and review-stage statuses into the server manifest, **Evidence Vault Lineage Digest**, **Evidence Vault Lineage Recovery Packet**, and **Evidence Vault Control Coverage** summary. The manifest updates with per-item hashes and a bundle SHA-256; after server sync, use **Download Vault Manifest JSON**, **Download Lineage Digest JSON**, **Refresh Lineage Recovery Packet**, and **Download Recovery Packet JSON** to export persisted metadata-only vault handoff and recovery artifacts when rejected records or missing manifest lineage block final handoff. The Evidence Audit Trail records local evidence creation, template application, source-gap requests/refreshes, edits, removals, and a JSON export. Check **Evidence Retention Readiness** before vault sync; private-key-like material, API-key-like credentials, and raw KYC references block Evidence Vault sync until replaced with metadata-only summaries. Use **Download Remediation Queue JSON** and **Download Disposal Runbook JSON** to hand off P0 delete/replace and P1 metadata-confirmation work without raw secret, KYC, or personal-data content; the disposal runbook is workflow metadata only and does not claim deletion was performed.
...
11. Review **Integration Readiness Registry** on the main workspace to see which external adapters are ready, blocked, disabled, or waiting for policy approval. Use **Integration Enablement Dossier** and **Integration Enablement Gate** to download hashed metadata-only JSON artifacts that consolidate adapter readiness, provider, secret, object storage, parser, chain anchor, GRC destination policies, server receipt coverage, and the ordered recovery queue while keeping `externalEnablementAllowed: false`. Use Object Storage Policy Evaluation, Document Parser Policy Evaluation, Chain Anchor Policy Evaluation, and GRC Destination Policy Evaluation to produce metadata-only readiness reports before enabling any future storage, raw-document parser, wallet/chain, or ticket-system adapter. In **Policy Evaluation Receipts**, refresh server receipts, refresh the server receipt recovery packet, and download the receipt bundle or recovery packet JSON before any adapter enablement review. Use recovery buttons to open Model Connect, Evidence Ledger, Risk Audit, or Counsel Pack before enabling any future real provider, storage, OCR, GRC, or chain adapter.
12. Run **Secure Review Journey** to sync metadata-only evidence to the Phase 2 API, create a server Model Gateway receipt, open Human Review, record audit-log events, refresh the Server Model Run Ledger, download the Model Run Receipt JSON, and inspect the Server Audit Log Explorer. Gateway policy failures are shown as recoverable failure receipts with run IDs and remediation steps.
13. Open **Counsel Pack** to choose an export template, inspect the Export Safety Gate, edit the counsel question queue, update review status for each risk flag, confirm the Source Review Ledger, Source Freshness Board, Jurisdiction Readiness Digest, Local Counsel Routing Plan, and any open Source Update Approval Queue gates appear in the Markdown preview, save a Pack Version to capture manifest/Markdown hashes, Regulatory Source Pack hash, source review status, Jurisdiction Readiness Digest hash/status/handoff flag, and review-status diff, create a server export record from that latest version when the Phase 2 API is running, then download the server export receipt JSON, Markdown audit-prep packet with Regulatory Source Graph, Source Review Ledger, Source Freshness Board scheduling metadata, local counsel routes, source update approval gates, Model Intake summary and AI event hashes, use browser Print / Save PDF, download version JSON, download diff JSON, download manifest JSON, or create a simulated anchor receipt JSON for counsel/compliance review. If the gate detects private keys, API keys, or raw KYC materials, these export actions are disabled until the evidence is replaced with metadata-only summaries.

Workspace data is stored locally in browser `localStorage`. Local file evidence is hashed in the browser and stored as file metadata plus SHA-256, not raw file bytes. The Phase 2 API stores workspace, evidence metadata, model-run receipt, human-review, integration policy evaluation receipt, audit-log, and Counsel Pack export metadata records in SQLite when enabled. It does not store model credentials, raw KYC, personal data, raw evidence bytes, raw Markdown, or real chain transactions. API keys for live browser model calls are held in browser state and are not persisted. Model Intake JSON, Evidence Audit Trail JSON, Evidence Retention Policy JSON, Evidence Retention Remediation Queue JSON, Evidence Disposal Runbook JSON, Evidence Vault Manifest JSON, Evidence Vault Lineage Digest JSON, Evidence Vault Lineage Recovery Packet JSON, Manifest Drift Report JSON, Demo Smoke Checklist JSON, Demo Runbook JSON, Judge Handoff Bundle JSON, Integration Readiness Registry output, Integration Enablement Dossier JSON, Integration Enablement Gate JSON, Integration Policy Evaluation Receipts and receipt bundle JSON, GRC Destination Policy JSON, GRC Ticket Export JSON, Model Gateway Evaluation JSON, Model Run Receipt JSON, Audit Log Export JSON, Counsel Pack version JSON, Counsel Pack server export receipt JSON, Counsel Pack export recovery packet JSON, Counsel Handoff Checklist JSON, Export Safety Inventory JSON, Model Gateway receipts, server export records, and model-run ledger exports store hashes and metadata, not credentials or raw Markdown content. Evidence Retention Readiness redacts detected secrets in snippets and blocks Evidence Vault sync for private-key-like material, credential-like tokens, and raw KYC references; wallet-address identifiers and personal-data references remain warning-level review signals for counsel handoff. The remediation queue carries only redacted snippets, priority, next action, and queue SHA-256. The disposal runbook carries only workflow tasks, verification prompts, and a runbook SHA-256; it does not upload raw evidence or perform deletion. Integration Readiness Registry, Integration Enablement Dossier, and Integration Enablement Gate expose disabled, needs-policy, ready, and blocked adapter states only; they do not call external providers, upload objects, run OCR, create GRC tickets, or write to a chain. GRC Destination Policy and GRC Ticket Export create local/server metadata-only JSON artifacts only; they do not create external Jira, Linear, ServiceNow, or GRC system records. The Evidence Vault API blocks invalid status transitions such as directly reactivating rejected or superseded records; users must use replacement recovery so lineage remains visible. The Export Safety Gate, Manifest Drift Guard, Export Safety Inventory, Counsel Handoff Checklist, and Judge Handoff Bundle redact detected secrets in previews/downloads and block or flag export handoff for the same blocked data classes. The anchor receipt is a local simulation for manifest handoff only.
...
The API defaults to `http://127.0.0.1:8787` and currently exposes `GET /api/health`, `GET /api/preflight`, Model Gateway adapter readiness, provider policy and secret policy evaluation routes, Object Storage Policy Evaluation, Document Parser Policy Evaluation, Chain Anchor Policy Evaluation, GRC Destination Policy Evaluation, Workspace create/read/update routes, multipart Evidence Vault upload/list/update/replacement/manifest/lineage-digest/lineage-recovery routes, mock Model Gateway run create/list/lookup/recovery routes, Human Review create/update/list plus queue-view/recovery-packet routes, Source Review sync/list/packet routes, Source Approval sync/list/packet routes, Counsel Pack export-record create/list/read plus recovery-packet routes, Integration Policy Evaluation receipt list/bundle/recovery routes, filtered Audit Log listing/export/recovery routes, and Prisma/SQLite persistence for workspace/evidence/model/review/integration-policy/source-review/source-approval/export/audit records. Health and API preflight behavior lives in `server/systemRoutes.ts`; `/api/preflight` returns a metadata-only capability, route-family, implemented-route, limitation, and stable report-hash artifact generated by `src/lib/apiPreflight.ts`, keeping `server/app.ts` focused on composition and shared hooks. Evidence uploads are hashed server-side and responses stay metadata-only. Evidence Vault manifests are generated from persisted metadata through `src/lib/evidenceVaultManifest.ts`, with stable item ordering, lineage/status/version/control-link hashing, no raw file bytes, and no source-note body content in the JSON; the Evidence Ledger can download that server manifest after sync. `GET /api/workspaces/:workspaceId/evidence-lineage-digest` returns active/replaced/rejected counts, lineage links, linked control/risk IDs, manifest hash, next actions, and a stable digest hash without exposing raw evidence or source-note body text. `GET /api/workspaces/:workspaceId/evidence-lineage-recovery` returns a metadata-only recovery packet with open rejected evidence IDs, missing-manifest state, linked controls/risks, non-empty next actions, and a stable packet hash without raw evidence, source-note body text, or replacement-reason body text. Duplicate evidence hashes are blocked with recoverable errors, rejected vault records can be superseded by replacement metadata with parent lineage and a replacement reason, and invalid status transitions return recovery guidance before any write or audit-log mutation. Model Gateway policy routes evaluate provider readiness and secret-policy controls without accepting provider credentials or enabling external proxying. `POST /api/integrations/object-storage/policy` evaluates metadata-only object-storage controls without raw uploads, credentials, storage buckets, or adapter enablement; it always returns `externalObjectStorageAllowed: false`. `POST /api/integrations/document-parser/policy` evaluates metadata-only document parser controls without raw document bytes, raw document body, credentials, OCR execution, parser adapters, or legal-decision output; it always returns `externalDocumentParsingAllowed: false`. `POST /api/integrations/chain-anchor/policy` evaluates metadata-only simulated anchor controls without wallet keys, signed transactions, raw evidence, raw KYC, credentials, or blockchain submission; it always returns `externalChainAnchoringAllowed: false` and `anchorMode: simulated-only`. `POST /api/integrations/grc-destination/policy` evaluates metadata-only GRC destination controls without API keys, webhook secrets, raw ticket bodies, raw KYC, personal data, or external ticket creation; it always returns `externalGrcTicketCreationAllowed: false`. When those integration policy requests include a workspace ID and the Phase 2 API repository is active, the response also includes a metadata-only `evaluationRecord`, persists that receipt, and writes an audit-log entry; `GET /api/workspaces/:workspaceId/integration-policy-evaluations` lists the receipts without raw policy, context, evidence, credential, or ticket body content, `GET /api/workspaces/:workspaceId/integration-policy-evaluations/bundle` returns a metadata-only receipt bundle with persisted receipt hashes, missing policy IDs, disabled external enablement state, next actions, and a stable bundle hash, and `GET /api/workspaces/:workspaceId/integration-policy-evaluations/recovery` returns missing, blocked, needs-policy, and stale receipt recovery actions with a stable packet hash while keeping external enablement disabled. `POST /api/workspaces/:workspaceId/source-reviews` persists metadata-only source review records and audit-log entries while keeping `matchingBehaviorChanged: false`; `GET /api/workspaces/:workspaceId/source-reviews` lists those records, and `GET /api/workspaces/:workspaceId/source-reviews/packet` returns record hashes, ledger hashes, review counts, next actions, and a stable packet hash without raw source bodies or reviewer-note body text. `POST /api/workspaces/:workspaceId/source-approvals` persists metadata-only source approval records and audit-log entries from the Source Update Approval Queue while keeping `matchingBehaviorChanged: false`; `GET /api/workspaces/:workspaceId/source-approvals` lists those records, and `GET /api/workspaces/:workspaceId/source-approvals/packet` returns record hashes, queue hashes, approval-gate counts, next actions, and a stable packet hash without raw source bodies or reviewer-note body text. `GET /api/workspaces/:workspaceId/audit-log` supports actor, action, target type, and target ID filters from the Secure Review Workspace without returning raw payloads, credentials, raw KYC, personal data, or legal conclusions; `GET /api/workspaces/:workspaceId/audit-log/export` returns a redacted metadata-only export with event entry hashes, integrity chain hash, export hash, boundary findings, remediation, next actions, and the Not legal advice boundary, while `GET /api/workspaces/:workspaceId/audit-log/recovery` returns a stable recovery packet for empty, blocked, or review-needed Audit Log handoff states. Model Gateway runs enforce Redaction Gate status, allowed data classes, purpose, human-review owner, credential/KYC blockers, and adapter policy. Successful runs persist payload hash, response hash, source evidence hash, provider metadata, retry state, and human-review status, then automatically create a model-run Human Review request with audit-log metadata; the Secure Review Workspace can refresh persisted run summaries through the Server Model Run Ledger. Failed or blocked runs persist safe failure receipts with run IDs, retry state, error codes, and remediation steps without returning raw payloads. `GET /api/workspaces/:workspaceId/model-runs/recovery` returns a metadata-only recovery packet with a stable packet hash, blocked/retry/human-review/ready counts, redacted per-run remediation, and next actions across saved gateway runs. Human Review queue views group filtered review records by target type, status, reviewer, and next action for evidence, model-run, risk-flag, clause-match, and counsel-pack targets without representing review as legal approval; `GET /api/workspaces/:workspaceId/reviews/recovery` returns the same redacted server recovery packet with a stable packet hash, target-specific next action, and the Not legal advice boundary for rejected or returned records. Evidence-target decisions sync Evidence Vault status, and model-run decisions sync Model Gateway human-review status plus audit-log metadata. Counsel Pack export records persist version number, artifact name, manifest hash, artifact hash, review summary, source count, Regulatory Source Pack hash, source review status, and the Not legal advice boundary without storing raw Markdown/PDF content; `GET /api/workspaces/:workspaceId/exports/counsel-pack/recovery` returns a stable metadata-only recovery packet for blocked review items, stale source review, jurisdiction-readiness blockers, and open evidence-review actions across saved export records. The backend only enables the local mock model adapter in this phase; OpenAI-compatible and enterprise-proxy adapters are listed as disabled placeholders even when secret policy controls evaluate ready, until a separate adapter enablement review is approved. It does not persist uploaded file bytes, store model credentials, process KYC, call external model providers, upload to object storage, run OCR, create external GRC tickets, or write to a blockchain.

Server Human Review recovery packets also expose a validated `nextActions` list through the queue view and `GET /api/workspaces/:workspaceId/reviews/recovery` so returned/rejected server review records have artifact-level recovery actions in API responses, direct workbench refreshes, downloads, demo-smoke checks, and the Secure Review UI.

Demo API preflight now requires non-empty, non-blank next actions on Human Review queue/recovery, Model Gateway run recovery, Evidence Vault lineage digest, Evidence Vault lineage recovery, Counsel Pack export recovery, Audit Log export/recovery, Integration Policy receipt bundle, and Integration Policy receipt recovery artifacts before the judge path can report ready. Not legal advice.

Source Review Ledger persistence is also available through `POST /api/workspaces/:workspaceId/source-reviews`, `GET /api/workspaces/:workspaceId/source-reviews`, and `GET /api/workspaces/:workspaceId/source-reviews/packet`. It stores metadata-only source review records with a stable ledger hash, audit-log entry, `matchingBehaviorChanged: false`, and the Not legal advice boundary; the packet route adds record hashes, ledger hashes, review counts, and recovery actions without raw source bodies, reviewer-note body text, or regulatory matching changes.

Source Approval persistence is available through `POST /api/workspaces/:workspaceId/source-approvals`, `GET /api/workspaces/:workspaceId/source-approvals`, and `GET /api/workspaces/:workspaceId/source-approvals/packet`. It stores metadata-only source approval gate records with a stable queue hash, audit-log entry, `matchingBehaviorChanged: false`, and the Not legal advice boundary; the packet route adds record hashes, queue hashes, approval and metadata-required counts, and recovery actions without raw source bodies, reviewer-note body text, legal approvals, or regulatory matching changes.
