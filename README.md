...
- Server Model Gateway provider policy API, workbench refresh flow, receipts, automatic Human Review queueing for completed gateway output, Model Gateway Evaluation artifacts, and Audit Log Export artifacts with payload hash, response hash, source evidence hash, provider policy metadata, human-review status, retry/error state, audit actions, before/after hashes, and remediation steps without returning raw payloads or credentials.
