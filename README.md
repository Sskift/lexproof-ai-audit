...
Policy Evaluation Receipts show the latest server-side integration policy evaluations for the workspace, including policy ID, control readiness, report hash, context hash, and disabled external capability state. The workbench can refresh persisted receipts from `GET /api/workspaces/:workspaceId/integration-policy-evaluations` after a reload or handoff, then download a metadata-only receipt bundle with a stable bundle hash, missing policy IDs, blocked/needs-policy counts, and validated non-empty next actions for adapter enablement review. They are audit preparation metadata only and do not store credentials, raw evidence, raw KYC, personal data, or external write commands. Not legal advice.
