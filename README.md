...
11. Review **Integration Readiness Registry** on the main workspace to see which external adapters are ready, blocked, disabled, or waiting for policy approval. Use Object Storage Policy Evaluation and Document Parser Policy Evaluation to produce metadata-only readiness reports before enabling any future storage or raw-document parser adapter. Use recovery buttons to open Model Connect, Evidence Ledger, Risk Audit, or Counsel Pack before enabling any future real provider, storage, OCR, GRC, or chain adapter.
