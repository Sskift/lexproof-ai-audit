...
Server Human Review recovery packets also expose a validated `nextActions` list through the queue view and `GET /api/workspaces/:workspaceId/reviews/recovery` so returned/rejected server review records have artifact-level recovery actions in API responses, downloads, demo-smoke checks, and the Secure Review UI.
