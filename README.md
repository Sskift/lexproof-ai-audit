...
Once the API is running, the judge smoke CLI checks required npm scripts, demo files, screenshot assets, `/api/health`, `/api/preflight`, and safe GET route families for Model Gateway, Evidence Vault manifest, Human Review queue, Source Review Ledger, Source Approval Queue, Counsel Pack exports, Audit Log, and Integration Policy Evaluation receipts without using private credentials:
