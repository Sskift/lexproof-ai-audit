...
8. Counsel Pack template selection, Source Review Ledger and Source Update Approval Queue preview, version save with Regulatory Source Pack hash, server export record creation, Markdown export, Manifest JSON, and simulated anchor receipt.
...
11. Review **Integration Readiness Registry** on the main workspace to see which external adapters are ready, blocked, disabled, or waiting for policy approval. Use Object Storage Policy Evaluation, Document Parser Policy Evaluation, and Chain Anchor Policy Evaluation to produce metadata-only readiness reports before enabling any future storage, raw-document parser, or wallet/chain adapter. Use recovery buttons to open Model Connect, Evidence Ledger, Risk Audit, or Counsel Pack before enabling any future real provider, storage, OCR, GRC, or chain adapter.
