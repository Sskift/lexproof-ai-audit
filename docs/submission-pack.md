...
7. Open **Sources** and download **Judge Handoff Bundle JSON**, then **Submission Pack JSON** plus **Demo Runbook JSON**, to show the judge-facing bundle hash, child artifact hashes, pack hash, Regulatory Source Coverage hash/status, Demo Runbook hash, Counsel Handoff Checklist status, known limitations, demo readiness, and clean-clone path.
