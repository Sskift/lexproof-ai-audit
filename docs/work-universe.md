...
- Request validation, typed error responses, and consistent audit logging. The shared typed error helper is implemented in `server/apiError.ts` and currently wired into Workspace, Model Gateway, Human Review, and Counsel Pack export failures with stable error codes and the audit-prep boundary.
