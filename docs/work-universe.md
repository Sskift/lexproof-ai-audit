...
- Demo Smoke Checklist and Demo Runbook artifacts in `src/lib/demoReadiness.ts`, `src/lib/demoRunbook.ts`, `DemoReadinessPanel`, and `SubmissionPackPanel`: generate metadata-only clean-clone command checks, scenario paths, screenshots, API preflight status, limitations, Not legal advice boundaries, a Judge Demo Readiness smoke-checklist JSON download, a stable Demo Runbook JSON download from Project Workspace or Sources, and Sources-level handoff tracking through Submission Pack and Export Safety Inventory.
