...
- Thailand, Indonesia, Malaysia, and Philippines jurisdiction packs now route those source-backed controls into policy controls, evidence-ready status, and local-counsel handoff roles when a custom project includes `Thailand`, `Indonesia`, `Malaysia`, `Philippines`, `ID`, `MY`, or the `PH` jurisdiction alias. Not legal advice. Source-control-aware evidence matching prevents a Malaysia or Philippines register from covering other jurisdiction controls only through broad custody, AML, or exchange keywords.
