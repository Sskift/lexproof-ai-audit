...
- Local Human Review operations with reviewer assignment, due dates, saved status history, source clause-match refresh actions, linked evidence updates, downloadable timeline JSON with audit log IDs, and a downloadable Recovery Packet JSON for returned/rejected review work with a stable packet hash and redacted reviewer notes.
- Project Workspace local persistence recovery for corrupt, invalid-shape, or unsafe saved workspace snapshots, plus unsafe project-metadata autosave blocking so credentials, private keys, raw KYC, direct personal identifiers, and wallet addresses are not written into local project snapshots.
- Editable Evidence Ledger with review-stage statuses, rejected-record replacement requests, empty-state Evidence Intake Guidance, local file metadata hashing, evidence templates, audit trail JSON, deterministic Evidence Manifest, Manifest Drift Guard, Evidence Vault control coverage summary, Evidence Vault Lineage Digest with stable digest hash and JSON download, simulated anchor receipt, Counsel Pack export templates, Export Safety Gate data-boundary blocker, Counsel Handoff Checklist with Manifest Drift Guard status, Evidence Recertification Queue status, Evidence Vault Lineage Digest readiness, a stable checklist hash, and JSON download, local Counsel Pack version history with manifest/Markdown/source-pack hashes, diff metadata, standalone diff JSON download, and Phase 2 server export records for the latest Pack Version.
- Risk Audit source citation controls that attach Regulatory Source Graph citations, source URLs, local-counsel routes, coverage status, and top open source-linked evidence requests to each active deterministic risk flag, and now carry the same per-risk source-lineage metadata into the Counsel Pack Markdown handoff without making legal conclusions. Jurisdiction Readiness Digest metadata now also carries into the Counsel Pack Markdown handoff with per-jurisdiction blockers, digest hashes, source freshness blockers, and local counsel routing context.
- Manual Evidence Ledger drafts now show an Evidence Draft Boundary Preview for blocked or review-needed data classes, with redacted recovery guidance before downstream vault, model, or export handoff.
- Local file evidence intake now performs a metadata boundary check before hashing, blocks raw KYC, credential-like, and private-key-like filenames or metadata, and shows redacted recovery guidance without uploading or storing raw file bytes.
- Jurisdiction checklist and jurisdiction packs for initial US/EU/UK/Singapore/Hong Kong/Japan/Canada/Australia/South Korea/India/Thailand/Indonesia/Malaysia/Philippines/South Africa/Switzerland/Germany/UAE/Brazil routing, plus source-backed New York NYDFS virtual-currency custody and US GENIUS Act payment stablecoin issuer routing inside the US review lane, EU MiCA ART/EMT stablecoin issuer routing inside the EU review lane, and UK qualifying stablecoin issuer routing inside the UK review lane.
- Demo script, screenshots, Demo Scenario Library launcher with source-control proof signals, Workspace Cockpit Handoff export, Workspace Journey rail, Source Review Packet export, and integration tests for the full secure review journey, Counsel Pack template selection, version-history export path, server export-record path, command-center Regulatory Control Matrix export, Jurisdiction Readiness Digest export, and seeded judge paths.
- Generated Submission Pack JSON in Sources with pack hash, manifest hash, Regulatory Source Pack hash, Regulatory Source Coverage hash/status, Demo Runbook hash, export safety summary, demo readiness, required assets, feature-to-theme mapping, known limitations, and Not legal advice boundary.
- Generated Judge Handoff Bundle JSON in Sources with Submission Pack, Demo Runbook, Export Safety Inventory, and Counsel Handoff Checklist child artifact hashes/statuses, recovery queue, stable bundle hash, and Not legal advice boundary, plus a Handoff Recovery Playbook that orders blockers, missing artifacts, stale drift targets, and review-needed handoff steps into clickable workspace recovery actions with a stable playbook hash; open playbook steps now also surface in the first-screen Workspace Action Queue, route back to Sources, and export as a separate metadata-only Action Queue JSON with a stable queue hash.
...
| Deterministic risk audit | Risk flags are reproducible, source-linked, and evidence-dependent | Risk Audit, issue cards, remediation queue, GRC export | `src/lib/auditEngine.ts`, `src/lib/riskExplainers.ts`, `src/lib/sourceCitationControls.ts`, `src/lib/riskEvidence.ts`, `src/lib/grcTicketExport.ts` | Trigger/non-trigger tests, coverage tests, export boundary tests |
...
- Regulatory graph evidence matching is control-aware: evidence with explicit `regulatory control: control-*` metadata is scoped to those controls, while user-entered evidence without control IDs can still fall back to keyword matching. This prevents one jurisdiction/control template from covering another source control solely through broad custody, AML, or issuer keywords.
...
- New York NYDFS BitLicense and custody customer-protection routing is implemented for US virtual-currency business activity and custody review using NYDFS virtual currency business licensing metadata, 23 NYCRR Part 200 references, and the September 30, 2025 updated custodial-structures guidance. The `tokenized-yield-rwa` evidence template now carries `control-us-nydfs-bitlicense-custody-customer-protection` and a New York NYDFS BitLicense / custody customer-protection register so New York resident access, BitLicense or trust-company route, customer virtual-currency segregation, beneficial-interest disclosure, sub-custody approval, reconciliation, books-and-records, and no-proprietary-use gaps can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions or raw KYC.
- US GENIUS Act payment stablecoin issuer routing is implemented for US payment stablecoin issuer review using Treasury implementation materials, the GENIUS Act statutory citation, and OCC Bulletin 2026-3 metadata. The `tokenized-yield-rwa` evidence template now carries `control-us-genius-payment-stablecoin-issuer-regime` and US GENIUS Act permitted-issuer/reserve/redemption plus BSA/AML/sanctions registers so permitted payment stablecoin issuer route, state or federal oversight, reserve assets, redemption, monthly disclosure, custody, insolvency-priority, transaction-monitoring, sanctions, BSA/AML, and no-raw-KYC handoff gaps can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- US CFTC Ooki DAO / DeFi derivatives-platform routing is implemented for DAO governance review using CFTC Release No. 8590-22 and CFTC v. Ooki DAO default-judgment metadata. The `dao-governance-multisig` evidence template now carries `control-us-cftc-ooki-dao-defi-derivatives-platform` and a DAO derivatives platform boundary and BSA/CIP review register so leveraged or margined retail commodity transaction assumptions, DeFi trading platform scope, FCM activity, commodity-interest handling, US user access, BSA/CIP boundaries, control transfer, successor DAO governance-member participation, website/domain operation, proposal execution, and no-raw-customer-record exclusions can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- EU MiCA decentralised DAO / CASP perimeter routing is implemented for DAO governance review using Regulation (EU) 2023/1114 Recital 22 and Article 2(1) metadata. The `dao-governance-multisig` evidence template now carries `control-eu-mica-decentralised-casp-perimeter` and an EU MiCA decentralisation / CASP perimeter register so decentralisation claims, intermediary or operator control, front-end/admin-key handling, protocol upgrades, EU user access, crypto-asset service perimeter, custody/trading/exchange/advice exclusions, marketing owner, responsible owner, and no-raw-customer-record evidence can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
...
- EU DLT Pilot Regime market-infrastructure routing is implemented for tokenized financial-instrument and private-credit note review using Regulation (EU) 2022/858 metadata. The `tokenized-yield-rwa` evidence template now carries `control-eu-dlt-pilot-regime-market-infrastructure` and an EU DLT Pilot Regime market infrastructure perimeter register so DLT financial-instrument assumptions, DLT MTF/TSS/SS routes, competent-authority or ESMA handoff metadata, admitted-instrument perimeter, settlement workflow, safekeeping, client disclosure, and liability-safeguard evidence can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions or raw investor records.
- EU MiCA ART/EMT stablecoin issuer routing is implemented for EU stablecoin issuer review using EBA ART/EMT MiCA materials and ESMA MiCA single rulebook metadata. The `tokenized-yield-rwa` evidence template now carries `control-eu-mica-art-emt-stablecoin-issuer-regime` and EU MiCA ART/EMT issuer authorisation / white-paper plus reserve / redemption / recovery registers so asset-referenced token or e-money token classification, issuer route, home Member State, competent-authority handoff, white paper, reserve composition, reserve custody, redemption, recovery plan, redemption plan, and no-raw-customer-record evidence can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- UK qualifying stablecoin issuer routing is implemented for UK stablecoin issuer review using FCA PS26/10 and BoE/FCA joint-approach metadata. The `tokenized-yield-rwa` evidence template now carries `control-uk-fca-qualifying-stablecoin-issuer-regime` and UK qualifying stablecoin issuer permission / disclosure plus backing / safeguarding / redemption registers so UKQS issuer route, regulated activity assumptions, admission or distribution scope, disclosures, backing assets, safeguarding, reconciliation, redemption, liquidity, recordkeeping, systemic-transition handoff, and no-raw-customer-record evidence can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
...
- Singapore MAS PSN02 AML/CFT and PS-G03 DPT customer-asset safeguarding routing is implemented for Singapore digital payment token custody review using MAS Notice PSN02, Guidelines to Notice PSN02, and MAS PS-G03 metadata. The `tokenized-yield-rwa` evidence template now carries `control-sg-mas-psn02-dpt-aml-cft` and `control-sg-mas-dpt-customer-asset-safeguards` so AML/CFT risk assessment, CDD, sanctions, transaction monitoring, data-redaction/model handoff, customer asset segregation, custody disclosure, reconciliation, transfer-control, and no-raw-KYC evidence gaps can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- Hong Kong HKMA stablecoin issuer routing is implemented for fiat-referenced stablecoin issuer review using the HKMA regulatory regime page, Stablecoins Ordinance licensing note, Supervisory Guideline, AML/CFT Guideline, and 2026 licensed-issuer press release metadata. The `tokenized-yield-rwa` evidence template now carries `control-hk-hkma-stablecoin-issuer-regime` and Hong Kong stablecoin licensing / reserve / redemption / AML-CFT registers so HKMA licence or application route, regulated stablecoin activity, reserve assets, full backing, redemption, attestation, reporting, AML/CFT, recordkeeping, complaints, user-protection, and no-raw-KYC exclusions can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions. SFC VATP custody matching is now fact-triggered so stablecoin issuer reviews do not inherit exchange-platform custody controls unless VATP facts are present.
- Hong Kong SFC tokenised authorised investment product and secondary-trading routing is implemented for tokenised product review using SFC Circular Ref. 26EC22 and official SFC Circular Ref. 26EC23 metadata. The `tokenized-yield-rwa` evidence template now carries `control-hk-sfc-tokenised-investment-products-secondary-trading` and Hong Kong SFC tokenised product authorisation / consultation, ownership / smart-contract controls, and secondary-trading fair-pricing / liquidity registers so SFC-authorised product scope, prior consultation or approval route, ownership recordkeeping, operational soundness, cybersecurity/data-privacy/business-continuity controls, smart-contract integrity, price-deviation alerts, indicative NAV, market-maker arrangements, investor disclosure, notification, contingency handling, and no-raw-investor-record exclusions can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
...
- Thailand SEC digital asset business custody and AMLO AML/CDD routing is implemented for Thailand digital asset exchange, broker, dealer, custodial-wallet-provider, custody, and AML/CDD review using Thailand SEC digital asset business operator metadata, the Emergency Decree on Digital Asset Businesses, SEC custody amendment notices, and AMLO AML/CFT measures. The `tokenized-yield-rwa` evidence template now carries `control-th-sec-digital-asset-business-custody-aml` and a Thailand digital asset custody and AML/CDD register so licensing route, operating-system readiness, client-asset records, transfer approvals, daily reconciliation, client-asset-use prohibition, financial-institution AML status, CDD/EDD, beneficial ownership, high-risk customer, internal-control, training, reporting-owner, and raw-KYC/customer-record/wallet-secret exclusion gaps can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- Indonesia OJK digital financial asset and crypto asset trading routing is implemented for Indonesia digital financial asset trading operators using OJK licensing metadata, the January 10 2025 Bappebti-to-OJK transfer notice, POJK 27/2024 as amended by POJK 23/2025, SEOJK 20/2024, the OJK licensed/registered operator whitelist press release, and POJK 16/2025 governance/main-party competence metadata. The `tokenized-yield-rwa` evidence template now carries `control-id-ojk-digital-financial-asset-crypto-trading` and an Indonesia OJK digital financial asset trading and whitelist register so OJK licensing route, PAKD/CPAKD status, SPRINT, whitelist, official app/site channel, product or instrument registration, reporting, business-plan owner, main-party competence/compliance assessment, governance, integrity, consumer-protection, and raw-KYC/customer-record/wallet-secret exclusion gaps can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- Malaysia SC/BNM digital asset exchange, custody, and AML/CFT routing is implemented for Malaysia digital asset exchange, digital broker, Digital Asset Custodian, IEO, tradeable-asset, and digital-currency reporting-institution review using Securities Commission Malaysia Digital Assets and Guidelines on Digital Assets metadata plus Bank Negara Malaysia AML/CFT - Digital Currencies (Sector 6) metadata. The `tokenized-yield-rwa` evidence template now carries `control-my-sc-bnm-digital-asset-exchange-custody-aml` and a Malaysia digital asset exchange custody and AML/CFT register so RMO-DAX, digital broker, DAC registration route, official app/site channel, tradeable asset and Shariah review assumptions, custody safeguarding, BNM reporting-institution scope, customer identification, CDD/EDD, beneficial ownership, STR, compliance officer, recordkeeping, transparency, and raw-KYC/customer-record/wallet-secret exclusion gaps can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- Philippines BSP VASP/CASP risk-management routing is implemented for Philippines VASP custody and counterparty review using BSP Circular No. 1108 and BSP Memorandum No. M-2026-003 metadata. The `tokenized-yield-rwa` evidence template now carries `control-ph-bsp-vasp-casp-risk-management-aml` and a Philippines BSP VASP custody and AML/CFT risk-management register so BSP registration or Certificate of Authority route, VASP/CASP counterparty checks, VA exchange, VA transfer, safekeeping, VA custodian, wallet security, retail/offshore access, proof-of-registration validation, due diligence/EDD, payment transparency, transaction monitoring, STR, recordkeeping, staff training, and raw-KYC/customer-record/wallet-secret exclusion gaps can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- South Africa FSCA/FIC CASP licensing and Travel Rule routing is implemented for South Africa crypto asset service provider review using FSCA General Notice 1350 of 2022, the FSCA New Financial Services Providers page, and FIC Directive 9 metadata. The `tokenized-yield-rwa` evidence template now carries `control-za-fsca-fic-casp-licensing-travel-rule` and a South Africa CASP licensing and Travel Rule RMCP register so crypto-asset financial-product treatment, CASP/FSP licence route, advice/intermediary/investment-management scope, business model, operational ability, fit-and-proper owner, ordering/intermediary/recipient CASP roles, counterparty due diligence, secure transmission, incomplete-transfer handling, unhosted-wallet policy, RMCP, recordkeeping, and raw-KYC/customer-record/wallet-secret exclusion gaps can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- Thailand, Indonesia, Malaysia, Philippines, and South Africa jurisdiction packs now route those source-backed controls into policy controls, evidence-ready status, and local-counsel handoff roles when a custom project includes `Thailand`, `Indonesia`, `Malaysia`, `Philippines`, `South Africa`, `ID`, `MY`, `PH`, or the `ZA` jurisdiction alias. Not legal advice. Source-control-aware evidence matching prevents a Malaysia, Philippines, or South Africa register from covering other jurisdiction controls only through broad custody, AML, or exchange keywords.
- UK FCA cryptoasset AML/MLR registration and Travel Rule routing is implemented for UK cryptoasset exchange-provider and custodian-wallet-provider review using FCA AML/CTF regime, registration expectations, Travel Rule expectations, and FCA Financial Crime Guide metadata. The `tokenized-yield-rwa` evidence template now carries `control-uk-fca-cryptoasset-aml-registration-travel-rule` and a UK FCA cryptoasset AML registration and Travel Rule register so activity scope, MLR registration, business plan, ownership/control, MLRO, AML/CTF/CPF framework, BWRA/CRA, CDD/EDD, sanctions, transaction monitoring, SAR escalation, Travel Rule originator/beneficiary data flow, record retrieval, outsourcing, reviewer-owner, and no-raw-KYC/customer-record/wallet-secret exclusion gaps can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- Germany BaFin/MiCAR CASP custody routing is implemented for Germany crypto-asset service provider custody review using BaFin/Bundesbank MiCAR supervisory information and MiCAR Articles 60, 62, and 75 metadata. The `tokenized-yield-rwa` evidence template now carries `control-de-bafin-micar-casp-custody-authorisation` and a Germany BaFin MiCAR CASP custody and Article 60/62 register so service scope, BaFin route, Article 60 notification, Article 62 authorisation assumptions, Article 75 custody policy, client-position records, segregation, return-process, means-of-access handling, private-key exclusion, and no-raw-customer-record evidence can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- Swiss FINMA ICO token-classification and FINMA Guidance 06/2024 stablecoin routing is implemented for Swiss stablecoin issuer review. The `tokenized-yield-rwa` evidence template now carries `control-ch-finma-stablecoin-guidance-06-2024` and Swiss stablecoin issuer / AML transfer-risk registers so payment/utility/asset/hybrid token classification, offering/prospectus intake, redemption claims, reserve assets, bank guarantees, banking-law and collective-investment perimeter assumptions, AML, sanctions, holder identification, transfer-risk, and no-raw-customer-record exclusions can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- UAE VARA operating routing is implemented for Dubai virtual-asset activity-scope, licensing, compliance management, AML/CFT, client virtual asset custody, proof-of-reserves, reconciliation, and local-counsel handoff review using the VARA Virtual Assets and Related Activities Regulations 2023 plus the VARA Compliance and Risk Management Rulebook. VARA 2024 marketing controls now require marketing-specific trigger facts so operating/custody demos do not inherit promotion-only gaps. The `tokenized-yield-rwa` evidence template carries `control-uae-vara-va-regulations-activity-scope` and `control-uae-vara-compliance-risk-management` without creating legal conclusions.
- ABA Formal Opinion 512 routing is implemented for AI legal workflow professional-responsibility review. The `ai-compliance-workflow` evidence template now carries `control-us-aba-formal-opinion-512-generative-ai-law-practice` and a US legal AI ethics and professional responsibility register so competence, confidentiality, client-communication, supervisory-review, candor, fee-treatment, reviewer-training, outside-provider oversight, and no-confidential-matter-text gaps can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
...
- Singapore IMDA / AI Verify Foundation Agentic AI governance routing is implemented for AI legal workflow review using the Model AI Governance Framework for Agentic AI and AI Verify Testing Framework metadata. The `ai-compliance-workflow` evidence template now carries `control-sg-imda-agentic-ai-governance` and two Singapore agentic AI registers so action-space, tool permissions, access controls, autonomy level, human approval checkpoints, logging, monitoring, user responsibility, training, AI Verify documentary evidence, and no-raw-matter-text exclusions can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- NYC Local Law 144 AEDT employment-decision routing is implemented for AI legal workflow review when project facts mention NYC, AEDT, hiring, promotion, bias audit, notice, accommodation or alternative selection process, or data-retention request handling. The `ai-compliance-workflow` evidence template now carries `control-us-nyc-local-law-144-aedt-employment-decision-governance` and two NYC AEDT registers so hiring/promotion scope, independent bias audit, public summary, distribution date, selection/scoring rates, impact ratios, 10-business-day notice, job qualifications or characteristics, accommodation or alternative-process instructions, AEDT data/source/retention request handling, and no raw applicant or employee records can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- Colorado SB26-189 ADMT consequential-decision routing is implemented for AI legal workflow review when project facts mention Colorado, ADMT, or consequential-decision scoping. The `ai-compliance-workflow` evidence template now carries `control-us-colorado-admt-consequential-decision-governance` and two Colorado ADMT registers so covered ADMT scope, technical documentation, intended uses, known limitations, deployer notice, adverse-outcome explanation, personal-data correction, meaningful human review, reconsideration, and three-year record-retention evidence can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions or raw personal data.
- California CCPA / CPPA ADMT consumer-rights routing is implemented for AI legal workflow review when project facts mention California, CCPA, CPPA, ADMT, significant-decision scoping, access, opt-out, risk assessment, or human involvement. The `ai-compliance-workflow` evidence template now carries `control-us-california-ccpa-admt-consumer-rights-governance` and two California ADMT registers so scope, significant-decision mapping, logic assumptions, known limitations, output use, human involvement, risk-assessment ownership, access request, opt-out, verification, secure response, non-retaliation, and no-raw-personal-data evidence can flow into the regulatory graph, Evidence Vault control coverage, and counsel handoff without legal conclusions.
- AI legal workflow source-control coverage is implemented for EU AI Act Article 4 / Chapter III review routing, EU AI Act Article 50 transparency disclosure routing, EU AI Act Article 6(2) / Annex III point 8(a) administration-of-justice and ADR perimeter routing, and UK ICO AI data protection guidance. The `ai-compliance-workflow` evidence template carries `control-eu-ai-act-ai-literacy-governance`, `control-eu-ai-act-article-50-transparency-disclosure`, `control-eu-ai-act-administration-justice-adr-perimeter`, and `control-uk-ico-ai-data-protection-governance` source references so the AI scenario can show human oversight, source lineage, AI-user disclosure, AI-generated output labelling, editorial control, justice/ADR perimeter, high-risk oversight, fundamental-rights review, redaction, and reviewer decision-log gaps without legal conclusions.
...
- `src/lib/regulatorySourceApproval.ts`, `src/lib/regulatorySourceApprovalSync.ts`, `src/lib/regulatorySourceApprovalClient.ts`, `server/sourceApprovalRoutes.ts`, and `RegulatoryCommandCenter` for downloadable, server-syncable, and refreshable metadata-only Source Update Approval Queue records with review-due source approvals, metadata gates, audit-log entries, persisted-record UI, and a rule that source updates cannot change matching behavior until counsel or compliance review records refreshed source metadata.
- Source Review Ledger now exposes an as-of date control so planned handoff dates can surface review-due source approvals, and the Workspace Action Queue can focus the Source Update Approval Queue as a recoverable first-screen action without changing source matching behavior.
...
- `src/lib/regulatorySourceCoverage.ts` and `RegulatoryCommandCenter` for a downloadable metadata-only source coverage report with jurisdiction-level source counts, source-review freshness, open evidence requests, local counsel roles, recovery actions, stable coverage hash, and Not legal advice boundary.
...
- `src/lib/manifestDrift.ts` and **Manifest Drift Guard** compare the current Evidence Manifest with refreshed Evidence Vault local-content hash lineage, the latest saved Counsel Pack version, and the latest server export record. The guard shows stale/missing targets, recovery actions, a stable report hash, and downloadable metadata-only JSON without source-note bodies, raw evidence, legal conclusions, or compliance labels.
...
- Manifest Drift Guard shows when evidence changed after the saved Counsel Pack version, server export record, or refreshed Evidence Vault metadata and gives a concrete recovery action before final handoff.
...
- Exportable review timeline with audit log IDs, plus a Human Review Recovery Packet for returned/rejected items with target-specific recovery actions, redacted notes, stable packet hash, and Not legal advice wording.
...
- Versioned Counsel Pack records with manifest hash, review status, Regulatory Source Pack hash, source review status, Jurisdiction Readiness Digest hash/status/handoff flag, source pack, export timestamp, and metadata-only server export records.
...
- Download receipts for manifest JSON, model-run receipts, server Counsel Pack export records, evidence audit trail, and simulated/real anchor receipts. The Secure Review Workspace now downloads a metadata-only Model Run Receipt JSON from the Server Model Run Ledger with a stable receipt hash, provider policy metadata, remediation steps, and the Not legal advice boundary. Counsel Pack server export rows now download a metadata-only Export Record Receipt JSON with the record hashes, Jurisdiction Readiness Digest hash/status/handoff flag, review summary, recovery action, stable receipt hash, and Not legal advice boundary.
- Export diff view between pack versions. Counsel Pack version rows now expose standalone Diff JSON downloads for non-baseline versions without raw Markdown content.
...
- Sources-level Export Safety Inventory in `src/lib/exportSafetyInventory.ts` and `ExportSafetyInventoryPanel` consolidates Evidence Manifest, Manifest Drift Guard, Evidence Vault Lineage Digest, Evidence Recertification Queue, Evidence Disposal Runbook, Model Gateway Evaluation, source pack, Counsel Pack, source-review, Source Update Approval Queue, Source Freshness Board, Demo Runbook, local-counsel, GRC, integration dossier, Integration Enablement Gate, and Submission Pack artifact readiness into one redacted hashed JSON handoff with Not legal advice wording.
...
- Sources-level Handoff Recovery Playbook in `src/lib/handoffRecoveryPlaybook.ts` and `HandoffRecoveryPlaybookPanel` consolidates Judge Handoff Bundle, Export Safety Inventory, and Counsel Handoff Checklist recovery states into ordered clickable recovery steps with redacted reasons, target workbench tabs, metadata-only JSON download, a stable playbook hash, and Not legal advice wording.
...
- API Preflight Report in `src/lib/apiPreflight.ts` and `server/systemRoutes.ts`: exposes `GET /api/preflight` as a metadata-only capability, route-family, implemented-route, limitation, and stable report-hash artifact for the secure review journey. It covers Model Gateway, Model Connect provider policy, Evidence Vault manifest, Human Review queue, Source Review Ledger, Source Approval Queue, Counsel Pack exports, Audit Log, and Integration Policy Evaluation receipts without storing credentials, accepting raw evidence/KYC, calling external providers, changing regulatory matching behavior, or making external writes.
- Repository interfaces with Prisma implementations and memory implementations for tests.
- Request validation, typed error responses, and consistent audit logging. The shared typed error helper is implemented in `server/apiError.ts` and currently wired into Workspace, Evidence Vault, Model Gateway, Human Review, Counsel Pack export, Audit Log filter failures, and Integration Policy malformed-payload failures with stable error codes and the audit-prep boundary. `src/lib/auditLogClient.ts` and `SecureReviewWorkspace` now expose filtered metadata-only Audit Log refreshes by actor, action, target type, and target ID without changing route contracts. `src/lib/modelGatewayRunClient.ts`, `src/lib/modelGatewayRunReceipt.ts`, and the Server Model Run Ledger now refresh persisted Model Gateway run summaries and export metadata-only model-run receipts after Secure Review Journey completion without exposing raw payloads or credentials.
...
- Retention and deletion policy before storing raw files. The first Evidence Ledger retention gate is implemented in `src/lib/retentionPolicy.ts`, `src/lib/evidenceRetentionRemediation.ts`, `src/lib/evidenceDisposalRunbook.ts`, and `EvidenceLedger`: it blocks Evidence Vault sync for private-key-like material, API-key-like credentials, and raw KYC references, shows a hashed P0/P1/P2 remediation queue, and exports metadata-only retention policy, remediation queue, and disposal runbook JSON without performing deletion.
- Audit log export with actor, action, target, timestamp, before/after hashes, per-event entry hashes, a stable export hash, an integrity chain hash, and non-secret summaries. The first Secure Review Audit Log Export is implemented in `src/lib/auditLogExport.ts` and `SecureReviewWorkspace` with metadata-only JSON download; the Server Audit Log Explorer can refresh filtered server records through the Phase 2 API with sanitized error recovery.
...
- Integration Enablement Dossier and Integration Enablement Gate in `src/lib/integrationEnablementDossier.ts` and `IntegrationReadinessPanel`: consolidate adapter readiness plus provider, secret, object storage, document parser, chain anchor, GRC destination policy reports, persisted server-side Integration Policy Evaluation receipt summaries, per-policy server receipt coverage, and an ordered enablement recovery queue into hashed metadata-only JSON handoffs with sanitized blockers, next actions, Not legal advice boundaries, and `externalEnablementAllowed: false`.
- Integration Policy Evaluation Receipts in `src/lib/integrationPolicyEvaluation.ts`, `src/lib/integrationPolicyEvaluationClient.ts`, `server/integrationPolicyRoutes.ts`, `server/reviewWorkspaceRepository.ts`, and `IntegrationReadinessPanel`: persist workspace-scoped report/context/policy hashes, refresh durable receipt records from the Phase 2 API, and write audit-log entries for server-evaluated object storage, document parser, chain anchor, and GRC destination policies without storing raw policy payloads, credentials, raw evidence, raw KYC, personal data, or external write commands.
...
- Demo Scenario Library in `src/data/demoScenarios.ts`, `src/lib/demoScenarioLibrary.ts`, and `DemoScenarioLibrary`: maps realistic synthetic sample profiles into short judge-ready routes, expected artifacts, focus tags, and recommended workbench tabs. The seeded set now includes RWA launch, DAO review, public-source review, AI legal workflow review, Brazil VASP source review, Swiss FINMA stablecoin review, US GENIUS Act stablecoin review, EU MiCA ART/EMT stablecoin review, UK qualifying stablecoin issuer review, Singapore DPT custody / MAS PSN02 AML-CFT review, Hong Kong VATP custody review, Hong Kong HKMA stablecoin issuer review, Japan crypto custody review, Canada CTP custody review, Australia digital asset review, Korea VASP user protection review, India VDA PMLA review, Thailand digital asset custody review, Indonesia OJK crypto trading review, Malaysia digital asset exchange review, Philippines VASP custody review, South Africa CASP Travel Rule review, UK cryptoasset AML review, Germany MiCAR custody review, UAE VARA operating review, and marketing claims review paths. The validation layer rejects unknown samples, missing Not legal advice boundaries, weak paths, empty artifacts, raw KYC, private-key, seed-phrase, and live-key demo text.
- Submission Pack artifact in `src/lib/submissionPack.ts` and `SubmissionPackPanel`: generates a metadata-only judge packet with stable pack hash, manifest/source-pack/source-coverage/Demo Runbook hashes, source coverage status and open source-evidence count, export safety summary, demo readiness, required assets, hackathon mapping, known limitations, and JSON download.
- Demo Smoke Checklist, `npm run demo:smoke` CLI preflight, and Demo Runbook artifacts in `src/lib/demoReadiness.ts`, `src/lib/demoRunbook.ts`, `scripts/demo-smoke.mjs`, `DemoReadinessPanel`, and `SubmissionPackPanel`: generate metadata-only clean-clone command checks, repository file checks, screenshot asset checks, safe Phase 2 API health, `/api/preflight`, and route-family preflight status, scenario paths, screenshots, limitations, Not legal advice boundaries, a Judge Demo Readiness smoke-checklist JSON download, a stable Demo Runbook JSON download from Project Workspace or Sources, and Sources-level handoff tracking through Submission Pack and Export Safety Inventory.
