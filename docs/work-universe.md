...
- Editable Evidence Ledger with review-stage statuses, rejected-record replacement requests, empty-state Evidence Intake Guidance, local file metadata hashing, evidence templates, audit trail JSON, deterministic Evidence Manifest, Evidence Vault control coverage summary, Evidence Vault Lineage Digest with stable digest hash and JSON download, simulated anchor receipt, Counsel Pack export templates, Export Safety Gate data-boundary blocker, Counsel Handoff Checklist with Evidence Recertification Queue status, Evidence Vault Lineage Digest readiness, a stable checklist hash, and JSON download, local Counsel Pack version history with manifest/Markdown/source-pack hashes, diff metadata, standalone diff JSON download, and Phase 2 server export records for the latest Pack Version.
...
- Sources-level Export Safety Inventory in `src/lib/exportSafetyInventory.ts` and `ExportSafetyInventoryPanel` consolidates Evidence Manifest, Evidence Vault Lineage Digest, Model Gateway Evaluation, source pack, Counsel Pack, source-review, Source Freshness Board, Demo Runbook, local-counsel, GRC, integration dossier, and Submission Pack artifact readiness into one redacted hashed JSON handoff with Not legal advice wording.
