...
- Sources-level Export Safety Inventory in `src/lib/exportSafetyInventory.ts` and `ExportSafetyInventoryPanel` consolidates Evidence Manifest, Manifest Drift Guard, Evidence Vault Lineage Digest, Model Gateway Evaluation, source pack, Counsel Pack, source-review, Source Update Approval Queue, Source Freshness Board, Demo Runbook, local-counsel, GRC, integration dossier, and Submission Pack artifact readiness into one redacted hashed JSON handoff with Not legal advice wording.
