...
- Fastify route modules split by domain: workspaces, evidence vault, model gateway, human review, audit log, exports. The first W7 split is implemented for Model Gateway routes in `server/modelGatewayRoutes.ts`; continue extracting the remaining domains without changing route contracts.
