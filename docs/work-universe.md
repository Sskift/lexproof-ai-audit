...
- Thailand, Indonesia, and Malaysia jurisdiction packs now route those source-backed controls into policy controls, evidence-ready status, and local-counsel handoff roles when a custom project includes `Thailand`, `Indonesia`, `Malaysia`, `ID`, or the `MY` jurisdiction alias. Not legal advice. Source-control-aware evidence matching prevents a Malaysia register from covering Thailand or Indonesia controls only through broad custody, AML, or exchange keywords.
