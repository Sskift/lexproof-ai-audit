...
- Integration Policy Evaluation Receipts in `src/lib/integrationPolicyEvaluation.ts`, `server/integrationPolicyRoutes.ts`, `server/reviewWorkspaceRepository.ts`, and `IntegrationReadinessPanel`: persist workspace-scoped report/context/policy hashes and audit-log entries for server-evaluated object storage, document parser, chain anchor, and GRC destination policies without storing raw policy payloads, credentials, raw evidence, raw KYC, personal data, or external write commands.
