...
- Risk Audit source citation controls that attach Regulatory Source Graph citations, source URLs, local-counsel routes, coverage status, and top open source-linked evidence requests to each active deterministic risk flag, and now carry the same per-risk source-lineage metadata into the Counsel Pack Markdown handoff without making legal conclusions.
